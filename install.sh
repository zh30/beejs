...
Environment variables:
  BEEJS_VERSION     Version tag to install (example: v0.1.2 or 0.1.2)
...
install_binary() {
  ...
  tmpdir=$(mktemp -d 2>/dev/null || mktemp -d -t bee)
  archive="$tmpdir/bee.tar.gz"
  url="https://github.com/${BEEJS_REPO}/releases/download/${version_tag}/bee-${version_tag}-${target}.tar.gz"
  ...
  if [ -f "$tmpdir/bee" ]; then
    src="$tmpdir/bee"
  else
    src=$(find "$tmpdir" -type f -name bee | head -n 1)
  fi

  [ -n "${src:-}" ] || fail "bee binary not found in archive"

  mkdir -p "$BEEJS_INSTALL_DIR"
  cp "$src" "$BEEJS_INSTALL_DIR/bee"
  chmod +x "$BEEJS_INSTALL_DIR/bee"
}
...
main() {
  ...
  echo "Beejs ${version_tag} installed to ${BEEJS_INSTALL_DIR}/bee"
  echo "Restart your shell or run:"
  echo "  export PATH=\"${BEEJS_INSTALL_DIR}:\$PATH\""
  echo "Verify with: bee --version"
}
