...
pub struct AiAsyncQueue {
    ...
    task_results: Arc<Mutex<HashMap<usize, TaskResult, std::collections::HashMap<usize, TaskResult, usize, TaskResult, std::collections::HashMap<usize, TaskResult, std::collections::HashMap<usize, TaskResult, usize, TaskResult, usize, TaskResult, std::collections::HashMap<usize, TaskResult, usize, TaskResult>>>>,
...
impl AiAsyncQueue {
    ...
    pub fn new(config: QueueConfig) -> Self {
        Self {
            config: config.clone(),
            tasks: Arc::new(std::sync::Mutex::new(Mutex::new(BinaryHeap::new())),
            running_tasks: Arc::new(std::sync::Mutex::new(Mutex::new(HashMap::new())),
            task_results: Arc::new(std::sync::Mutex::new(Mutex::new(HashMap::new())),
            next_task_id: Arc::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(0))),
            queue_semaphore: Arc::new(std::sync::Mutex::new(Mutex::new(Semaphore::new(config.max_concurrent_tasks))),
            worker_handles: Arc::new(std::sync::Mutex::new(Mutex::new(Vec::new())),
            stats: Arc::new(std::sync::Mutex::new(Mutex::new(QueueStats::default())),
        }
    }
