...
impl AiBatchProcessor {
    ...
    pub fn new(config: BatchConfig) -> Self {
        ...
        Self {
            config,
            pending_tasks: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(VecDeque::new())))),
            active_batches: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(0))))),
            batch_semaphore: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(Semaphore::new(max_concurrent_batches))))),
            next_task_id: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(0))))),
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(BatchStats::default())))),
            results: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(Vec::new())))),
        }
    }
    ...
    async fn try_process_batch(&self) {
        ...
        tokio::spawn(async move {
            ...
            *active_batches.lock().unwrap() += 1;
            ...
            // 将批次结果添加到全局结果集合
            {
                let mut all_results = results.clone();clone();clone();clone();clone();clone();lock().unwrap();
