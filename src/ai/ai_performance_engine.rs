...
impl AiPerformanceEngine {
    ...
    pub fn new(config: AiPerformanceEngineConfig) -> Self {
        Self {
            config: config.clone(),
            metrics_history: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(VecDeque::with_capacity(config.prediction_window))))),
    ...
    async fn train_online(&self) {
        ...
        *self.is_training.lock().unwrap() = true;
        ...
        //     // 训练预测器
        //     {
        //         let mut predictor = predictor.clone();clone();clone();lock().unwrap();
