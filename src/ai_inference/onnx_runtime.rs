...
impl EngineFactory for OnnxEngineFactory {
    async fn create(&self, engine_type: EngineType) -> Result<Box<dyn InferenceEngine>> {
        ...
        // 创建引擎统计
        let stats: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(EngineStats {
        ...
        let engine: _ = OnnxEngine {
            session: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(session))))),
            gpu_accelerator,
            optimizer,
            stats,
...
impl StreamManager {
    ...
    fn new(stream_count: usize) -> Result<Self> {
        ...
        Ok(StreamManager {
            stream_count,
            active_streams: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(streams))))),
        })
    }
