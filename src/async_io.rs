...
impl AsyncIoManager {
    ...
    pub fn new(max_concurrent_tasks: usize) -> Self {
        Self {
            ...
            active_tasks: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::atomic::AtomicUsize::new(0)))))),
            stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(tokio::sync::Mutex::new(IoStats::default()))))),
        }
    }

    /// 异步读取多个文件
    pub async fn read_files_concurrent(&self, mut paths: Vec<String>) -> Vec<AsyncFileRead> {
        let semaphore: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(tokio::sync::Semaphore::new(self.max_concurrent_tasks))))));
