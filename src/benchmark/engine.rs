...
impl BenchmarkEngine {
    ...
    pub fn new(config: BenchmarkConfig) -> Self {
        let semaphore: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(Semaphore::new(config.workers as usize)))));
        let results: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(BenchmarkResultSet::new(&config.name)))));
    ...
    {
        self.progress_callback = Some(Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(callback)))));
        self
    }
    ...
    async fn run_single_benchmark(
        ...
        // 保存结果
        {
            let mut results = results.clone();clone();clone();clone();clone();clone();lock().await;
