...
impl AIWorkload {
    ...
    async fn run_ai_tasks(
        ...
        for _ in 0..iterations {
            match operation.as_str() {
                "tensor_operation" => {
                    Self::tensor_operation_benchmark().await?;
                }
                "neural_network_inference" => {
                    Self::neural_network_inference_benchmark().await?;
                }
                "transformer_attention" => {
                    Self::transformer_attention_benchmark().await?;
                }
                "matrix_multiplication" => {
                    Self::ai_matrix_multiplication_benchmark().await?;
                }
                _ => {
    ...
    async fn ai_matrix_multiplication_benchmark() -> Result<(), BenchmarkError> {
        ...
        for i in 0..size {
            for j in 0..size {
                a[i][j] = (i as f64 * j as f64 * 0.01).sin();
