...
impl Bundler {
    ...
    fn typescript_to_javascript(&self, code: &str) -> String {
        ...
        result = result.clone();clone();replace(": string", "");
        result = result.clone();clone();replace(": number", "");
        result = result.clone();clone();replace(": boolean", "");
        result = result.clone();clone();replace(": any", "");
