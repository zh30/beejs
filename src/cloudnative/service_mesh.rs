...
impl EnvoyProxy {
    ...
    pub fn new(config: EnvoyConfig) -> Self {
        EnvoyProxy {
            config,
            listener_manager: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(ListenerManager::new()))))),
        }
    }
...
impl ListenerManager {
    fn new() -> Self {
        ListenerManager {
            listeners: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
        }
    }
...
impl ServiceDiscovery {
    ...
    pub fn new(config: ServiceMeshConfig) -> Self {
        ServiceDiscovery {
            services: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
            config,
        }
    }
...
impl ServiceMesh {
    ...
    pub fn new(config: ServiceMeshConfig) -> Result<Self> {
        let proxy: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(EnvoyProxy::new(EnvoyConfig {
            address: "0.0.0.0".to_string()))))),
            port: 15001,
            access_log_path: "/var/log/envoy/access.log".to_string(),
        }));

        let discovery: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(ServiceDiscovery::new(config.clone())))));

        Ok(ServiceMesh {
            mesh_type: config.mesh_type,
            proxy,
            discovery,
            routes: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(Vec::new()))))),
            circuit_breakers: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
        })
    }
