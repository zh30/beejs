...
impl DistributedTracer {
    ...
    pub fn new(
        ...
        let initial_stats = PerformanceStats {
            total_traces: 0,
            total_spans: 0,
        ...
        Self {
            ...
            performance_stats: Arc::new(RwLock::new(initial_stats)),
        }
    }
    ...
    pub async fn start_trace(&self, operation_name: String, service_name: String) -> TraceId {
        ...
        let trace = Trace::new(trace_id.clone(), root_span);
    ...
    pub async fn start_span(
        ...
        let parent_span_id = self.get_current_span_id(trace_id).await;
    ...
    async fn cleanup_old_traces(&self) -> Result<(), String> {
        let cutoff = Instant::now() - self.config.trace_retention;
    ...
    async fn update_performance_stats(&self) -> Result<(), String> {
        ...
        let p50_duration = if durations.len() > 1 {
            durations[durations.len() * 50 / 100]
        } else {
            Duration::from_millis(0)
        };

        let p90_duration = if durations.len() > 1 {
            durations[durations.len() * 90 / 100]
        } else {
            Duration::from_millis(0)
        };

        let p99_duration = if durations.len() > 1 {
