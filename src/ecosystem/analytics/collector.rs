...
impl AnalyticsCollector {
    ...
    pub fn new(max_events: usize) -> Self {
        Self {
            events: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(VecDeque::with_capacity(max_events)))))),
            benchmarks: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
            aggregator: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(DataAggregator::new()))))),
            max_events,
        }
    }
    ...
    async fn calculate_trends(&self, timeframe: &TimeFrame) -> Result<Vec<TrendData>, Box<dyn std::error::Error>> {
        let events: _ = self.events.read().await;
        let mut daily_counts: HashMap<DateTime<Utc, std::collections::HashMap<DateTime<Utc, DateTime<Utc, std::collections::HashMap<DateTime<Utc, std::collections::HashMap<DateTime<Utc, DateTime<Utc, DateTime<Utc, std::collections::HashMap<DateTime<Utc, DateTime<Utc>>>>>>>, u64> = HashMap::new();
...
impl DataAggregator {
    pub fn new() -> Self {
        Self {
            event_counts: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
            module_usage: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
        }
    }
