...
impl AnalyticsCollector {
    ...
    pub async fn generate_report(&self, timeframe: &TimeFrame) -> Result<AnalyticsReport, Box<dyn std::error::Error>> {
        ...
        // 获取顶级模块
        let top_modules = self.get_top_modules(10).await?;

        // 计算趋势
        let usage_trends = self.calculate_trends(timeframe).await?;
    ...
    pub async fn compare_performance(&self, module_a: &str, module_b: &str) -> Result<ComparisonResult, Box<dyn std::error::Error>> {
        ...
        let results_a = benchmarks.get(module_a).cloned().unwrap_or_default();
        let results_b = benchmarks.get(module_b).cloned().unwrap_or_default();
        ...
        let winner = if avg_score_a < avg_score_b { module_a.to_string() } else { module_b.to_string() };
        let performance_gain = ((avg_score_a - avg_score_b).abs() / avg_score_a.max(avg_score_b)) * 100.0;
    ...
    pub async fn get_module_performance(&self, module_id: &str) -> Result<ModulePerformanceReport, Box<dyn std::error::Error>> {
        let benchmarks = self.benchmarks.read().await;
        let results = benchmarks.get(module_id).cloned().unwrap_or_default();
        ...
        let avg_execution_time = results.iter().map(|r| r.execution_time.as_millis() as f64).sum::<f64>() / total_tests as f64;
        let avg_memory = results.iter().map(|r| r.memory_usage as f64).sum::<f64>() / total_tests as f64;
        let avg_cpu = results.iter().map(|r| r.cpu_usage).sum::<f64>() / total_tests as f64;
        let avg_score = results.iter().map(|r| r.score).sum::<f64>() / total_tests as f64;
    ...
    async fn get_top_modules(&self, limit: usize) -> Result<Vec<ModuleUsage>, Box<dyn std::error::Error>> {
        let module_usage = self.aggregator.module_usage.read().await;
    ...
    async fn calculate_trends(&self, timeframe: &TimeFrame) -> Result<Vec<TrendData>, Box<dyn std::error::Error>> {
        let events = self.events.read().await;
        let mut daily_counts: HashMap<DateTime<Utc>, u64> = HashMap::new();
    ...
    async fn calculate_performance_metrics(&self) -> Result<PerformanceMetrics, Box<dyn std::error::Error>> {
        ...
        let total_benchmarks = all_results.len() as u64;
        let avg_execution_time = all_results.iter().map(|r| r.execution_time.as_millis() as f64).sum::<f64>() / total_benchmarks as f64;
        let avg_memory_usage = all_results.iter().map(|r| r.memory_usage as f64).sum::<f64>() / total_benchmarks as f64;
        let peak_memory_usage = all_results.iter().map(|r| r.memory_usage).max().unwrap_or(0);

        // 找出最慢和最快的模块
        let mut module_times: HashMap<String, std::time::Duration> = HashMap::new();
        for result in &all_results {
            let current = module_times.entry(result.module_id.clone()).or_insert(std::time::Duration::from_millis(0));
...
mod tests {
    ...
    async fn test_track_usage() {
        let collector = AnalyticsCollector::new_with_defaults();
    ...
    async fn test_generate_report() {
        ...
        let report = collector.generate_report(&timeframe).await.unwrap();
    ...
    async fn test_benchmark_module() {
        let collector = AnalyticsCollector::new_with_defaults();
        let result = collector.benchmark_module("test-module", "performance-test").await.unwrap();
    ...
    async fn test_compare_performance() {
        ...
        let comparison = collector.compare_performance("module-a", "module-b").await.unwrap();
    ...
    async fn test_module_performance_report() {
        ...
        let report = collector.get_module_performance("test-module").await.unwrap();
    ...
    async fn test_cleanup_old_data() {
        ...
        let removed = collector.cleanup_old_data(Duration::days(5)).await.unwrap();
        assert_eq!(removed, 1);
    }
    ...
    async fn test_timeframe_constructors() {
        let day = TimeFrame::last_day();
        let week = TimeFrame::last_week();
        let month = TimeFrame::last_month();
