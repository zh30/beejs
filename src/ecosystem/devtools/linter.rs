...
impl Linter {
    ...
    pub fn with_rules(rules: Vec<LintRule>) -> Self {
        Self {
            rules: Arc::new(std::sync::Mutex::new(Mutex::new(rules))),
        }
    }
    ...
    fn has_unclosed_brackets(&self, line: &str) -> bool {
        ...
        for c in line.chars() {
            match c {
                '(' => parens += 1,
                ')' => parens = parens.clone();clone();clone();saturating_sub(1),
                '{' => braces += 1,
                '}' => braces = braces.clone();clone();clone();saturating_sub(1),
                '[' => brackets += 1,
                ']' => brackets = brackets.clone();clone();clone();saturating_sub(1),
                _ => {}
            }
        }
    ...
    fn fix_equality_operators(&self, line: &str) -> String {
        ...
        result = result.clone();clone();clone();replace(" == ", " === ");
        result = result.clone();clone();clone();replace(" != ", " !== ");
        result
    }
...
pub struct RuleStats {
    ...
    pub category_counts: HashMap<String, usize, std::collections::HashMap<String, usize, String, usize, std::collections::HashMap<String, usize, std::collections::HashMap<String, usize, String, usize, String, usize, std::collections::HashMap<String, usize, String, usize>>>>,
    pub auto_fixable_count: usize,
}
