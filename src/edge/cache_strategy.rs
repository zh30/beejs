...
impl EdgeCache {
    ...
    pub fn new(name: &str, l1_capacity: usize) -> Result<Self> {
        Ok(EdgeCache {
            name: name.to_string(),
            l1_cache: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(L1Cache::new(l1_capacity))))),
            l2_cache: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(L2Cache::new("regional", l1_capacity * 10)))))?)),
            l3_cache: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(L3Cache::new("central", l1_capacity * 100)))))?)),
            predictor: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(CachePredictor::new())))),
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(CacheStats {
                l1_hits: 0,
                l1_misses: 0,
                l2_hits: 0,
                l2_misses: 0,
...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_concurrent_cache_access() {
        let cache: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(EdgeCache::new("test-cache", 1000))))).unwrap());
        let mut handles = vec![];
