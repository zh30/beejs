...
impl OfflineExecutionEngine {
    ...
    pub async fn new(
        ...
        let engine: _ = OfflineExecutionEngine {
            runtime: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(None))),
            local_cache: Arc::new(std::sync::Mutex::new(Mutex::new(local_cache))),
            data_store: Arc::new(std::sync::Mutex::new(Mutex::new(data_store))),
            dependency_resolver: Arc::new(std::sync::Mutex::new(Mutex::new(DependencyResolver::new())),
            sync_manager: Arc::new(std::sync::Mutex::new(Mutex::new(super::local_cache::SyncManager::new())).await?),
        };
