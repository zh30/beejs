...
impl LogAggregator {
    ...
    pub fn new(config: LogAggregatorConfig) -> Result<Self> {
        ...
        let current_file: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(None)))));
        ...
        Ok(Self {
            config,
            current_file,
            log_queue: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(Vec::new())))),
            log_sender,
        })
    }
...
pub fn init_logger(config: LogAggregatorConfig) -> Result<Arc<LogAggregator>> {
    let aggregator: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(LogAggregator::new(config)))))?);
