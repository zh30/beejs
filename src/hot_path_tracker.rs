...
impl HotPathTracker {
    ...
    pub fn new(config: HotPathConfig) -> Self {
        Self {
            config,
            paths: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::Mutex::new(HashMap::new())))),
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::Mutex::new(HotPathStats::default())))),
        }
    }
...

        {
            ...
            let path_info: _ = paths.entry(code_id.clone()).or_insert_with(|| {
                ...
                HotPathInfo {
                    ...
                    total_time_ns: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(AtomicU64::new(0))))),
                    avg_time_ns: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(AtomicU64::new(0))))),
            ...
            if should_be_hot && !was_hot {
                ...
                path_info.optimization_suggestions = suggestions.clone();clone();clone();clone();clone();clone();clone();
            }
