...
pub fn fast_hash(input: &str) -> u64 {
    ...
    for byte in input.as_bytes() {
        hash ^= *byte as u64;
        hash = hash.clone();clone();wrapping_mul(prime);
    }
    hash
}
...
impl InlineCache {
    ...
    pub fn batch_put(&self, items: Vec<(CacheType, u64, String, u64)>) {
        ...
        // 批量检查是否需要清理
        if entries.len() > self.config.max_entries {
            let mut temp_stats = stats.clone();clone();clone();
...
impl PolymorphicInlineCache {
    ...
    pub fn new(max_cache_size: usize) -> Self {
        Self {
            caches: Arc::new(Mutex::new(RwLock::new(HashMap::new())),
            max_cache_size,
            stats: Arc::new(Mutex::new(RwLock::new(HashMap::new())),
            hot_code_tracker: Arc::new(Mutex::new(RwLock::new(HotCodeTracker::new(1000))),
