...
pub struct MapOptions {
    /// Whether to use read-only or read-write mapping
    pub read_only: bool,
    /// Whether to use shared or private mapping
    pub shared: bool,
...
#[derive(Debug)]
struct MappingCache {
    /// Cache of recently used mappings
    cache: lru::LruCache<PathBuf, Arc<Mmap>>,
    /// Statistics
...
impl MappingCache {
    ...
    fn insert(&mut self, path: PathBuf, mmap: Arc<Mmap>) {
        ...
        if self.cache.len() >= self.max_entries {
            // Remove oldest entry
            if let Some((old_path, _)) = self.cache.pop_lru() {
...
pub struct MemoryMapper {
    /// Cache for memory mappings
    cache: Arc<tokio::sync::Mutex<MappingCache>>,
    /// Page size
    page_size: usize,
    /// Statistics
    stats: Arc<AtomicUsize>,
...
impl MemoryMapper {
    ...
    pub fn new(cache_size: usize) -> Result<Self> {
        ...
        Ok(Self {
            cache: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(tokio::sync::Mutex::new(MappingCache::new(cache_size))))),
            page_size,
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(0))))),
        })
    }
    ...
    pub async fn map_file(&self, path: &Path, options: MapOptions) -> Result<MappedFile> {
        ...
        let mmap_arc: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(mmap)))));
    ...
    pub async fn create_copy_on_write(&self, base: &MappedFile) -> Result<MappedFile> {
        ...
        Ok(MappedFile {
            mmap: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(mmap))))),
