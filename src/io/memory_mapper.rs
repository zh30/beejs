...
impl MemoryMapper {
    ...
    pub fn new(cache_size: usize) -> Result<Self> {
        ...
        Ok(Self {
            cache: Arc::new(std::sync::Mutex::new(Mutex::new(tokio::sync::Mutex::new(MappingCache::new(cache_size))),
            page_size,
            stats: Arc::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(0))),
        })
    }
    ...
    pub async fn map_file(&self, path: &Path, options: MapOptions) -> Result<MappedFile> {
        ...
        let mmap_arc: _ = Arc::new(std::sync::Mutex::new(Mutex::new(mmap)));
    ...
    pub async fn create_copy_on_write(&self, base: &MappedFile) -> Result<MappedFile> {
        ...
        Ok(MappedFile {
            mmap: Arc::new(std::sync::Mutex::new(Mutex::new(mmap))),
