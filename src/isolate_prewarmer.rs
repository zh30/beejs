...
impl IsolatePrewarmer {
    ...
    pub fn new(max_prewarm: usize, config: PrewarmConfig) -> Result<Self> {
        ...
        let snapshot_manager: _ = Arc::new(std::sync::Mutex::new(Mutex::new(crate::v8_snapshot::SnapshotManager::new(snapshot_config)));

        Ok(Self {
            snapshot_manager,
            prewarmed_isolates: Arc::new(std::sync::Mutex::new(Mutex::new(Vec::new())),
            stats: Arc::new(std::sync::Mutex::new(Mutex::new(PrewarmStats::new())),
