...
impl Runtime {
    ...
    fn setup_console(&self, scope: &mut v8::ContextScope<v8::HandleScope>) -> Result<()> {
        ...
        let console_error_fn = console_error.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.error function"))?;
        let console_warn_fn = console_warn.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.warn function"))?;
        let console_info_fn = console_info.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.info function"))?;
        let console_debug_fn = console_debug.get_function(scope)
