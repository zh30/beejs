...
impl Runtime {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            verbose,
            isolate: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(isolate))))),
            context,
        })
    }
    ...
    fn execute_code_with_context(&self, code: &str, _base_dir: PathBuf) -> Result<String> {
        ...
        let result_str: _ = if result.is_undefined() {
            ...
            // For objects, arrays, etc., use to_json_string
            if let Some(json_str) = result.to_json_string(scope) {
    ...
    fn setup_console(&self, scope: &mut v8::ContextScope<v8::HandleScope>) -> Result<()> {
        ...
        let console_log: _ = v8::FunctionTemplate::new(scope, |scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut retval: v8::ReturnValue| {
            ...
            for i in 0..args.length() {
                ...
                let arg_str: _ = if arg.is_string() {
                    ...
                    // For objects, arrays, etc., use to_json_string
                    if let Some(json_str) = arg.to_json_string(scope) {
                        json_str.to_rust_string_lossy(scope)
                    } else {
                        "<unprintable>".to_string()
                    }
        ...
        let console_error_fn: _ = console_error.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.error function"))?;
        let console_warn_fn: _ = console_warn.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.warn function"))?;
        let console_info_fn: _ = console_info.get_function(scope)
            .ok_or_else(|| anyhow!("Failed to get console.info function"))?;
        let console_debug_fn: _ = console_debug.get_function(scope)
        ...
        console.set(scope, v8::String::new(scope, "error").unwrap().into(), console_error_fn.into())
            .map_err(|e| anyhow!("Failed to set console.error: {}", e))?;
        console.set(scope, v8::String::new(scope, "warn").unwrap().into(), console_warn_fn.into())
            .map_err(|e| anyhow!("Failed to set console.warn: {}", e))?;
        console.set(scope, v8::String::new(scope, "info").unwrap().into(), console_info_fn.into())
            .map_err(|e| anyhow!("Failed to set console.info: {}", e))?;
        console.set(scope, v8::String::new(scope, "debug").unwrap().into(), console_debug_fn.into())
            .map_err(|e| anyhow!("Failed to set console.debug: {}", e))?;
