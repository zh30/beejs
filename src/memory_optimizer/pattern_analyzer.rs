...
impl MemoryPatternAnalyzer {
    ...
    pub fn with_config(config: AnalyzerConfig) -> Self {
        Self {
            allocation_history: Arc::new(Mutex::new(RwLock::new(VecDeque::new())),
            active_allocations: Arc::new(Mutex::new(RwLock::new(HashMap::new())),
            statistics: Arc::new(Mutex::new(RwLock::new(AllocationStatistics {
                total_allocations: 0,
    ...
    async fn analyze_trends(&self) -> Vec<AllocationTrend> {
        ...
        // 按时间窗口分组
        let mut time_buckets: BTreeMap<DateTime<Utc, DateTime<Utc, DateTime<Utc, DateTime<Utc>, Vec<&AllocationRecord>> = BTreeMap::new();
