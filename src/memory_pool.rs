...
impl SmartMemoryPool {
    ...
    pub fn new(config: PoolConfig) -> Self {
        let pool: _ = Self {
            string_buffers: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(VecDeque::new())))),
            object_buffers: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(VecDeque::new())))),
            config: config.clone(),
    ...
    pub fn return_string_buffer(&self, mut buffer: StringBuffer) {
        ...
        if pool.len() < self.config.string_pool_size {
            ...
            // 池已满，计算节省的内存
            let saved_memory: _ = buffer.buffer.capacity();
            let mut stats = self.stats.lock().unwrap();
