...
impl DataStore {
    ...
    pub fn new(config: DataStoreConfig) -> Self {
        Self {
            config,
            memory_cache: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(VecDeque::new()))))),
            compressed_storage: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(HashMap::new()))))),
            query_index: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(QueryIndex {
                time_index: Vec::new()))))),
                type_index: HashMap::new(),
                last_update: Instant::now(),
            })),
            stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(DataStoreStats {
                total_data_points: 0,
                memory_cache_size: 0,
