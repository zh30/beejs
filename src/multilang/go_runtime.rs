...
pub struct GoRuntime {
    vm: Arc<GoVM>,
    goroutines: Arc<RwLock<HashMap<GoRoutineId, GoRoutine, std::collections::HashMap<GoRoutineId, GoRoutine, GoRoutineId, GoRoutine, std::collections::HashMap<GoRoutineId, GoRoutine, std::collections::HashMap<GoRoutineId, GoRoutine, GoRoutineId, GoRoutine, GoRoutineId, GoRoutine, std::collections::HashMap<GoRoutineId, GoRoutine, GoRoutineId, GoRoutine>>>>,
    bee_api: Arc<BeeAPI>,
...
impl GoRuntime {
    ...
    pub fn new(bee_api: Arc<BeeAPI>) -> Result<Self> {
        let vm: _ = Arc::new(std::sync::Mutex::new(Mutex::new(GoVM::new()))?);
        let goroutines: _ = Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()));
        let executor: _ = Arc::new(std::sync::Mutex::new(Mutex::new(GoExecutor {
...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_go_basic_execution() {
        let bee_api: _ = Arc::new(std::sync::Mutex::new(Mutex::new(BeeAPI {
...
"#;
    ...
    async fn test_go_bee_interop() {
        ...
        let bridge: _ = GoBeeBridge::new(
            Arc::new(std::sync::Mutex::new(Mutex::new(MockBeeRuntime))),
            Arc::new(std::sync::Mutex::new(Mutex::new(GoVM::new())).unwrap()),
        );
