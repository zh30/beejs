...
impl MultiLanguageRuntime {
    ...
    pub fn new() -> Self {
        MultiLanguageRuntime {
            python: None,
            go: None,
            rust: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RustOptimizer::new()))))),
        }
    }
        ...
        match language.to_lowercase().as_str() {
            "python" | "py" => {
                if let Some(python) = &self.python {
                    ...
                    Err(anyhow::anyhow!("Python runtime not initialized"))
                }
            }
            "go" => {
                if let Some(go) = &self.go {
                    ...
                    Err(anyhow::anyhow!("Go runtime not initialized"))
                }
            }
            "rust" | "rs" => {
...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_multilang_runtime() {
        ...
        let python_api: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(go_runtime::BeeAPI {
        ...
        // Test Go execution
        let go_api: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(go_runtime::BeeAPI {
            runtime: Arc::new(MockBeeRuntime)))))),
        });
