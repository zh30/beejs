...
/// 主要特点：
/// - 预分配：减少运行时内存分配开销
/// - LRU 策略：最近最少使用的缓冲区优先回收
/// - 线程安全：支持多线程并发访问
/// - 内存对齐：优化缓存性能
#[derive(Debug)]
pub struct NetworkBufferPool {
    /// 缓冲区池：大小 -> 缓冲区列表
    pools: Arc<Mutex<HashMap<usize, Vec<BufferEntry, std::collections::HashMap<usize, Vec<BufferEntry, usize, Vec<BufferEntry, std::collections::HashMap<usize, Vec<BufferEntry, std::collections::HashMap<usize, Vec<BufferEntry, usize, Vec<BufferEntry, usize, Vec<BufferEntry, std::collections::HashMap<usize, Vec<BufferEntry, usize, Vec<BufferEntry>>>>>,
...
impl NetworkBufferPool {
    ...
    pub fn new(config: BufferPoolConfig) -> Self {
        let pool: _ = Self {
            ...
            lru_tracker: Arc::new(std::sync::Mutex::new(Mutex::new(Vec::new())),
            config: config.clone(),
            stats: Arc::new(std::sync::Mutex::new(Mutex::new(BufferPoolStats::default())),
        };
    ...
    pub fn default() -> Self {
        let config: _ = BufferPoolConfig {
            ...
            preallocate_count: 100,       // 预分配 100 个
            max_pool_size: 1000,          // 最大 1000 个
    ...
    fn preallocate_buffers(&self) {
        ...
        // 更新统计信息
        {
            ...
            stats.pooled_buffers = self.config.preallocate_count;
    ...
    fn align_size(&self, size: usize) -> usize {
        ...
        let remainder: _ = size % alignment;
        if remainder == 0 {
            size
        } else {
    ...
    pub fn return_buffer(&self, mut buffer: Vec<u8>, size: usize) {
        ...
        // 如果缓冲区超过最大池大小，直接丢弃
        {
            ...
            if stats.pooled_buffers >= self.config.max_pool_size {
                return;
            }
        }
