...
impl MemoryMapper {
    ...
    pub fn new(config: Option<MemoryMapperConfig>) -> io::Result<Self> {
        ...
        Ok(Self {
            config,
            active_regions: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(HashMap::new())))),
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(MemoryMapperStats::default())))),
            next_region_id: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::atomic::AtomicU64::new(1))))),
            total_mapped_memory: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::atomic::AtomicUsize::new(0))))),
        })
    }
    ...
    fn perform_mmap(
        ...
        let flags: _ = if self.config.enable_huge_pages {
            ...
            #[cfg(target_os = "linux")]
            let flags: _ = flags | libc::MAP_HUGETLB;
