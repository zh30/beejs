...
impl ZeroCopyUdpSocket {
    ...
    pub fn new(socket: UdpSocket, buffer_size: usize, pool_size: usize) -> Self {
        let udp_socket: _ = Self {
            socket: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(socket)))))),
            packet_buffers: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(Vec::with_capacity(pool_size)))))),
            buffer_size,
            stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(UdpZeroCopyStats::default()))))),
        };
