...
impl AsyncZeroCopy {
    ...
    pub fn new(config: Option<ZeroCopyConfig>) -> io::Result<Self> {
        ...
        let monitor: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(ZeroCopyMonitor::new()))));

        Ok(Self {
            config,
            monitor,
            stats: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(TokioMutex::new(AsyncZeroCopyStats::default())))),
            active_tasks: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(TokioMutex::new(HashMap::new())))),
            next_task_id: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(std::sync::atomic::AtomicU64::new(1))))),
            semaphore: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(Semaphore::new(10))))), // 默认最多 10 个并发任务
        })
    }
