...
use std::collections::{HashMap, BTreeMap};
    ...
    fn test_setup_network_apis() {
        // TODO: 实现真正的缓冲池测试
        let _buffer_pool: _ = Arc::new(std::sync::Mutex::new(Mutex::new()));
        let _connection_pool: _ = Arc::new(std::sync::Mutex::new(Mutex::new(ConnectionPool::new(Default::default())).unwrap());
        let network_statistics: _ = Arc::new(std::sync::Mutex::new(Mutex::new(NetworkStats {
            total_connections: 0,
