...
pub fn setup_http_api(
    ...
    // createServer
    let create_server_func = v8::FunctionTemplate::new(scope, http_create_server_callback);
    ...
    // request
    let request_func = v8::FunctionTemplate::new(scope, http_request_callback);
    ...
    // get
    let get_func = v8::FunctionTemplate::new(scope, http_get_callback);
...
fn http_request_callback(
    ...
    let method_value = v8::String::new(scope, "GET").unwrap();
    req_obj.set(scope, method_key.into(), method_value.into());

    // url
    let url_key = v8::String::new(scope, "url").unwrap();
    let url_value = v8::String::new(scope, "/").unwrap();
    req_obj.set(scope, url_key.into(), url_value.into());

    // end
    let end_func = v8::FunctionTemplate::new(scope, http_req_end_callback);
