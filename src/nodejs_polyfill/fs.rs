...
pub fn register(scope: &mut v8::HandleScope, global: &v8::Local<v8::Object>) {
    ...
    // Read file
    let read_file_fn = v8::FunctionTemplate::new(scope, read_file).get_function(scope).unwrap();
    let read_file_key = v8::String::new(scope, "readFile").unwrap().into();
    fs_obj.set(scope, read_file_key, read_file_fn.into());

    // Write file
    let write_file_fn = v8::FunctionTemplate::new(scope, write_file).get_function(scope).unwrap();
    let write_file_key = v8::String::new(scope, "writeFile").unwrap().into();
    fs_obj.set(scope, write_file_key, write_file_fn.into());

    // Exists
    let exists_fn = v8::FunctionTemplate::new(scope, exists).get_function(scope).unwrap();
...
fn read_file(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut retval: v8::ReturnValue) {
    ...
    let path = path_arg.to_string(scope).unwrap().to_rust_string_lossy(scope);
...
fn write_file(scope: &mut v8::HandleScope, args: v8::FunctionCallbackArguments, mut retval: v8::ReturnValue) {
    ...
    let content = content_arg.to_string(scope).unwrap().to_rust_string_lossy(scope);
