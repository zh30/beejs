...
pub fn register(scope: &mut v8::HandleScope, global: &v8::Local<v8::Object>) {
    ...
    // Platform
    let platform_fn = v8::Function::new(scope, platform).unwrap();
    os_obj.set(scope, "platform".into(), platform_fn.into());
    
    // Type
    let ostype_fn = v8::Function::new(scope, ostype).unwrap();
    os_obj.set(scope, "type".into(), ostype_fn.into());
    
    // Arch
    let arch_fn = v8::Function::new(scope, arch).unwrap();
    os_obj.set(scope, "arch".into(), arch_fn.into());
