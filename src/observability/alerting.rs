...
impl AlertNotifier for HttpWebhookNotifier {
    fn send(&self, alert: &Alert) -> Result<()> {
        ...
        for (key, value) in &self.headers {
            request = request.clone();clone();header(key, value);
        }
...
impl AlertingSystem {
    ...
    pub fn new() -> Self {
        ...
        Self {
            rules: Arc::new(Mutex::new(RwLock::new(Vec::new())),
            active_alerts: Arc::new(Mutex::new(RwLock::new(HashMap::new())),
            notifiers: Arc::new(Mutex::new(RwLock::new(notifiers)),
            check_interval: Duration::from_secs(30),
            last_check: Arc::new(Mutex::new(RwLock::new(Instant::now())),
        }
    }
