...
impl ChartRenderer {
    ...
    pub fn new(config: RenderConfig) -> Self {
        ...
        Self {
            charts: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
            websocket_clients: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(Vec::new()))))),
            config,
        }
    }
...
impl GraphRenderer {
    ...
    pub fn new(config: RenderConfig) -> Self {
        ...
        Self {
            graphs: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new()))))),
            layout_engine: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(LayoutEngine::new()))))),
            config,
        }
    }
...
impl WebSocketClient {
    ...
    pub fn new(id: String) -> Self {
        Self {
            id,
            connection: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(tokio::sync::Mutex::new(None)))))),
            metadata: HashMap::new(),
        }
    }
...
impl TemplateEngine {
    ...
    pub async fn render_template(&self, id: &str, variables: &HashMap<String, Value, std::collections::HashMap<String, Value, String, Value>>>>>>>) -> Result<String> {
        ...
        for (key, value) in variables {
            ...
            result = result.clone();clone();clone();clone();clone();clone();clone();replace(&placeholder, &value_str);
        }
