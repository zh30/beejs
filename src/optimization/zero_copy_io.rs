...
impl ZeroCopyNetworkIO {
    ...
    pub fn stats(&self) -> NetworkIOStats {
        NetworkIOStats {
            ...
            buffer_count: self.buffer_pool.len(),
            avg_bytes_per_op: if self.operations.load(Ordering::Relaxed) > 0 {
                self.bytes_transferred.load(Ordering::Relaxed) as f64 /
                    self.operations.load(Ordering::Relaxed) as f64
            } else {
                0.0
            },
        }
...
impl<T: Clone> ZeroCopyMessageQueue<T> {
    ...
    pub fn stats(&self) -> MessageQueueStats {
        MessageQueueStats {
            messages_enqueued: self.enqueued.load(Ordering::Relaxed),
            messages_dequeued: self.dequeued.load(Ordering::Relaxed),
            current_size: self.queue.len() as u64,
        }
    }
}
...
impl ZeroCopyPipe {
    ...
    pub fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        ...
        if data.len() <= 512 {
            let result: _ = nix::unistd::write(self.write_fd, data);
            result.map_err(|e| io::Error::from_raw_os_error(e as i32))
        } else {
            // For large writes, use writev for scatter-gather
            let iov: _ = [nix::sys::uio::IoVec::from_slice(data)];
            let result: _ = nix::sys::uio::writev(self.write_fd, &iov);
            result.map_err(|e| io::Error::from_raw_os_error(e as i32))
        }
    }
...
impl ZeroCopyPerformanceMonitor {
    ...
    pub fn metrics(&self) -> ZeroCopyMetrics {
        ...
        ZeroCopyMetrics {
            ...
            total_bytes,
            total_time_ms: time_ns as f64 / 1_000_000.0,
            avg_throughput_mbps: if time_ns > 0 {
                (bytes as f64 / 1_000_000.0) / (time_ns as f64 / 1_000_000_000.0)
            } else {
                0.0
            },
            cache_hit_rate: if ops > 0 {
                self.cache_hits.load(Ordering::Relaxed) as f64 / ops as f64
            } else {
                0.0
            },
            ops_per_second: if time_ns > 0 {
                (ops as f64 * 1_000_000_000.0) / time_ns as f64
            } else {
                0.0
            },
        }
