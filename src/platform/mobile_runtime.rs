...
impl iOSRuntime {
    ...
    pub fn new(mobile_api: Arc<MobileAPI>) -> Result<Self> {
        let isolate_pool: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(IsolatePool::new(10)))));
...
impl IsolatePool {
    ...
    pub fn new(max_isolates: usize) -> Self {
        IsolatePool {
            isolates: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(Vec::new())))),
            max_isolates,
        }
    }
...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_ios_runtime() {
        ...
        let mobile_api: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(MobileAPI::new(MobilePlatform::iOS, capabilities)))));
    ...
    async fn test_android_runtime() {
        let jni_env: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(JNIEnv { _env: std::ptr::null_mut())))) });
