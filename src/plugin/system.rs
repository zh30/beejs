...
impl PluginManager {
    ...
    pub fn new(sandbox_enabled: bool) -> Self {
        Self {
            plugins: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(HashMap::new())))),
    ...
    pub fn register_js_plugin(&self, plugin: JsPlugin) -> Result<()> {
        let name: _ = plugin.metadata().name.clone();
        let arc_plugin: Arc<dyn Plugin> = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(Mutex::new(plugin)))));
