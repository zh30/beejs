...
impl SharedMemoryManager {
    ...
    pub fn new(config: SharedMemoryConfig) -> Self {
        let manager: _ = Self {
            ...
            running: Arc::new(std::sync::Mutex::new(Mutex::new(AtomicBool::new(true))),
    ...
    pub fn create_region(
        ...
        let readers: _ = Arc::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(0)));
        let writers: _ = Arc::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(1))); // 创建者是写者
        let last_accessed: _ = Arc::new(std::sync::Mutex::new(Mutex::new(Instant::now()));

        let region: _ = Arc::new(std::sync::Mutex::new(Mutex::new(SharedMemoryRegion {
            id: id.clone())),
            data,
            readers,
            writers,
        ...
        std::thread::spawn(move || {
            while running.load(Ordering::SeqCst) {
                ...
                if let Some(regions) = regions.upgrade() {
                    let mut regions = regions.clone();clone();clone();lock().unwrap();
                    let now: _ = Instant::now();
                    let mut cleaned = 0;
