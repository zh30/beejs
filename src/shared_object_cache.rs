...
impl SharedObjectCache {
    ...
    pub fn new(config: SharedObjectCacheConfig) -> Self {
        let cache: _ = Self {
            string_cache: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(StringInterner::new()))))),
            object_cache: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(LruCache::new(config.max_objects)))))),
            config: config.clone(),
            stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(SharedObjectCacheStats::default()))))),
    ...
    pub fn insert(&self, key: String, value: SharedValue) -> Arc<SharedObject> {
        let obj: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(SharedObject::new(value))))));
