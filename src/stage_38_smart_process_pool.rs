...
impl SmartProcessPool {
    ...
    pub fn new(base_config: ProcessPoolConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            load_balancer: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(SmartLoadBalancer {
                strategy: LoadBalancingStrategy::PerformanceBased,
                worker_performance_history: HashMap::new()))))),
                global_stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(GlobalPerformanceStats::default()))))),
            })),
            memory_manager: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(MemorySharingManager {
            ...
            predictor: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(PerformancePredictor {
                performance_history: VecDeque::new()))))),
                prediction_model: LinearRegressionModel::new(5), // 5个特征
                prediction_window: 100,
            })),
            pattern_analyzer: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(RwLock::new(TaskPattern::new()))))),
            monitoring_active: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(AtomicBool::new(false)))))),
