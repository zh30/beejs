...
impl LazyWebAPI {
    ...
    pub fn new() -> Self {
        Self {
            initialized_apis: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashSet::new())),
            initialization_queue: Arc::new(std::sync::Mutex::new(Mutex::new(Vec::new())),
            init_semaphore: Arc::new(std::sync::Mutex::new(Mutex::new(Semaphore::new(10))), // 最多 10 个并发初始化
            // web_api_registry: Arc::new(std::sync::Mutex::new(Mutex::new(WebApiRegistry::new())),
...
impl<T> LazyInitializer<T> {
    ...
    {
        Self {
            init_fn: Arc::new(std::sync::Mutex::new(Mutex::new(init_fn))),
...
pub struct OnDemandLoader {
    /// 已加载的模块
    loaded_modules: Arc<RwLock<HashMap<String, LoadedModule, std::collections::HashMap<String, LoadedModule, String, LoadedModule, std::collections::HashMap<String, LoadedModule, std::collections::HashMap<String, LoadedModule, String, LoadedModule, String, LoadedModule, std::collections::HashMap<String, LoadedModule, String, LoadedModule>>>>,
    /// 模块工厂
...
impl OnDemandLoader {
    ...
    pub fn with_factory(factory: Box<dyn ModuleFactory + Send + Sync>) -> Self {
        Self {
            loaded_modules: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new())),
...
impl StartupOptimizer {
    ...
    pub fn new(level: OptimizationLevel) -> Self {
        Self {
            lazy_web_api: Arc::new(std::sync::Mutex::new(Mutex::new(LazyWebAPI::new())),
            on_demand_loader: Arc::new(std::sync::Mutex::new(Mutex::new(OnDemandLoader::new())),
