...
impl CoverageTracker {
    pub fn new(config: CoverageTrackingConfig) -> Self {
        CoverageTracker {
            config,
            files: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(HashMap::new()))))),
        }
    }
...
pub fn init_global_tracker(config: CoverageTrackingConfig) -> Arc<CoverageTracker> {
    let tracker: _ = Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(CoverageTracker::new(config))))));
