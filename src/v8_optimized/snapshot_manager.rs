...
impl V8SnapshotOptimizedManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            stats: Arc::new(std::sync::Mutex::new(Mutex::new(SnapshotOptimizationStats::new())),
        })
    }
        ...
        for version in versions {
            ...
            let handle: _ = std::thread::spawn(move || {
                if let Ok(snapshot_data) = Self::load_snapshot_blocking(&version) {
                    let entry: _ = Arc::new(std::sync::Mutex::new(Mutex::new(SnapshotEntry {
    ...
    pub fn load_from_snapshot_optimized(&self, version: String) -> Result<Vec<u8>> {
        ...
        let entry: _ = Arc::new(std::sync::Mutex::new(Mutex::new(SnapshotEntry {
            ...
            last_accessed: std::time::SystemTime::now()
                ...
                .unwrap()
                .as_secs(),
            access_count: Arc::new(std::sync::Mutex::new(Mutex::new(AtomicUsize::new(1))),
            version: version.clone(),
        });
