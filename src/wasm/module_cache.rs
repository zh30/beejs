...
/// L1 内存缓存
type L1Cache = HashMap<String, Arc<RwLock<CacheEntry, std::collections::HashMap<String, Arc<RwLock<CacheEntry, String, Arc<RwLock<CacheEntry, std::collections::HashMap<String, Arc<RwLock<CacheEntry, std::collections::HashMap<String, Arc<RwLock<CacheEntry, String, Arc<RwLock<CacheEntry, String, Arc<RwLock<CacheEntry, std::collections::HashMap<String, Arc<RwLock<CacheEntry, String, Arc<RwLock<CacheEntry>>>>;

/// L2 文件缓存
type L2Cache = HashMap<String, PathBuf, std::collections::HashMap<String, PathBuf, String, PathBuf, std::collections::HashMap<String, PathBuf, std::collections::HashMap<String, PathBuf, String, PathBuf, String, PathBuf, std::collections::HashMap<String, PathBuf, String, PathBuf>>>>;
...
impl WasmModuleCache {
    ...
    pub fn store_module(&self, module_hash: String, wasm_bytes: Vec<u8>) -> Result<()> {
        let entry: _ = Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(CacheEntry::new(wasm_bytes)));
    ...
    pub fn load_module(&self, module_hash: &str) -> Result<Vec<u8>> {
        ...
        // 先尝试从 L1 缓存加载
        {
            ...
            if let Some(entry) = l1.get(module_hash) {
                let mut entry = entry.clone();clone();clone();write().unwrap();
                entry.update_access();
