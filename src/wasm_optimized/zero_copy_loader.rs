...
pub struct WasmZeroCopyLoader {
    ...
    memory_maps: Arc<RwLock<HashMap<String, Arc<Mmap, std::collections::HashMap<String, Arc<Mmap, String, Arc<Mmap, std::collections::HashMap<String, Arc<Mmap, std::collections::HashMap<String, Arc<Mmap, String, Arc<Mmap, String, Arc<Mmap, std::collections::HashMap<String, Arc<Mmap, String, Arc<Mmap>>>>>,
...
impl WasmZeroCopyLoader {
    ...
    pub fn new(cache_size: usize, prewarm_enabled: bool) -> Result<Self> {
        ...
        let loader: _ = Self {
            engine,
            module_cache: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(LruCache::new(NonZero::new(cache_size))).unwrap_or(NonZero::new(100).unwrap()),
            memory_maps: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(HashMap::new())),
            load_stats: Arc::new(std::sync::Mutex::new(Mutex::new(RwLock::new(LoadStats {
                total_loads: 0,
                cache_hits: 0,
                cache_misses: 0,
    ...
    async fn memory_map_file(&self, name: &str, file_path: &Path) -> Result<bool> {
        ...
        memory_maps.insert(name.to_string(), Arc::new(std::sync::Mutex::new(Mutex::new(mmap)));
    ...
    async fn create_module_from_memory(&self, name: &str, file_path: &Path) -> Result<Arc<Module>> {
        ...
        Ok(Arc::new(std::sync::Mutex::new(Mutex::new(module)))
    }
