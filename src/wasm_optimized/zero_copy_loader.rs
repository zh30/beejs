...
impl WasmZeroCopyLoader {
    ...
    pub fn new(cache_size: usize, prewarm_enabled: bool) -> Result<Self> {
        ...
        let loader: _ = Self {
            engine,
            module_cache: Arc::new(Mutex::new(RwLock::new(LruCache::new(NonZero::new(cache_size)).unwrap_or(NonZero::new(100).unwrap())),
            memory_maps: Arc::new(Mutex::new(RwLock::new(HashMap::new())),
            load_stats: Arc::new(Mutex::new(RwLock::new(LoadStats {
                total_loads: 0,
                cache_hits: 0,
                cache_misses: 0,
