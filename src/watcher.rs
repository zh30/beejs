...
impl HotReloader {
    ...
    pub fn with_config(config: WatcherConfig) -> Self {
        Self {
            config,
            stats: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(WatcherStats::new()))))),
            running: Arc::new(Mutex::new(Mutex::new(std::sync::Mutex::new(Mutex::new(AtomicBool::new(false)))))),
        }
    }
