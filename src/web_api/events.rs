...
pub fn setup_events_api(
    ...
    let event_target_key = v8::String::new(scope, "EventTarget").unwrap();
...
fn event_target_constructor_callback(
    ...
    let event_target_obj = v8::Object::new(scope);
...
mod tests {
    ...
    fn test_event_listener_management() {
        let target = EventTarget::new();
        let event_called = std::sync::Arc::new(std::sync::Mutex::new(false));
        let event_called_clone = event_called.clone();

        let listener = Box::new(move |event: &Event| {
        ...
        let result = target.dispatch_event(&event);
