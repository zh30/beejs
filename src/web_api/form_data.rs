...
fn form_data_constructor(
    ...
    let form_data_obj: _ = v8::Object::new(scope);

    // Add append method
    let append_key: _ = v8::String::new(scope, "append").unwrap();
    let append_template: _ = v8::FunctionTemplate::new(scope, form_data_append);
    let append_func: _ = append_template.get_function(scope).unwrap();
    form_data_obj.set(scope, append_key.into(), append_func.into());

    // Add delete method
    let delete_key: _ = v8::String::new(scope, "delete").unwrap();
    let delete_template: _ = v8::FunctionTemplate::new(scope, form_data_delete);
    let delete_func: _ = delete_template.get_function(scope).unwrap();
    form_data_obj.set(scope, delete_key.into(), delete_func.into());

    // Add get method
    let get_key: _ = v8::String::new(scope, "get").unwrap();
    let get_template: _ = v8::FunctionTemplate::new(scope, form_data_get);
    let get_func: _ = get_template.get_function(scope).unwrap();
    form_data_obj.set(scope, get_key.into(), get_func.into());

    // Add getAll method
    let get_all_key: _ = v8::String::new(scope, "getAll").unwrap();
    let get_all_template: _ = v8::FunctionTemplate::new(scope, form_data_get_all);
    let get_all_func: _ = get_all_template.get_function(scope).unwrap();
    form_data_obj.set(scope, get_all_key.into(), get_all_func.into());

    // Add has method
    let has_key: _ = v8::String::new(scope, "has").unwrap();
    let has_template: _ = v8::FunctionTemplate::new(scope, form_data_has);
    let has_func: _ = has_template.get_function(scope).unwrap();
    form_data_obj.set(scope, has_key.into(), has_func.into());

    // Add set method
    let set_key: _ = v8::String::new(scope, "set").unwrap();
    let set_template: _ = v8::FunctionTemplate::new(scope, form_data_set);
    let set_func: _ = set_template.get_function(scope).unwrap();
    form_data_obj.set(scope, set_key.into(), set_func.into());

    // Add entries method
    let entries_key: _ = v8::String::new(scope, "entries").unwrap();
    let entries_template: _ = v8::FunctionTemplate::new(scope, form_data_entries);
    let entries_func: _ = entries_template.get_function(scope).unwrap();
    form_data_obj.set(scope, entries_key.into(), entries_func.into());

    // Add keys method
    let keys_key: _ = v8::String::new(scope, "keys").unwrap();
    let keys_template: _ = v8::FunctionTemplate::new(scope, form_data_keys);
    let keys_func: _ = keys_template.get_function(scope).unwrap();
    form_data_obj.set(scope, keys_key.into(), keys_func.into());

    // Add values method
    let values_key: _ = v8::String::new(scope, "values").unwrap();
    let values_template: _ = v8::FunctionTemplate::new(scope, form_data_values);
    let values_func: _ = values_template.get_function(scope).unwrap();
    form_data_obj.set(scope, values_key.into(), values_func.into());

    // Add forEach method
    let for_each_key: _ = v8::String::new(scope, "forEach").unwrap();
    let for_each_template: _ = v8::FunctionTemplate::new(scope, form_data_for_each);
    let for_each_func: _ = for_each_template.get_function(scope).unwrap();
...
fn form_data_has(
    ...
    let false_val: _ = v8::Boolean::new(scope, false);
