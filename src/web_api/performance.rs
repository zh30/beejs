...
fn performance_time_origin_callback(
    ...
    let origin_millis = (system_now - elapsed_since_start).as_secs_f64() * 1000.0;
...
fn performance_measure_callback(
    ...
    let entry_type_key = v8::String::new(scope, "entryType").unwrap();
    let entry_type_val = v8::String::new(scope, "measure").unwrap();
    measure.set(scope, entry_type_key.into(), entry_type_val.into());

    let start_time_key = v8::String::new(scope, "startTime").unwrap();
    let start_time_val = v8::Number::new(scope, 0.0);
    measure.set(scope, start_time_key.into(), start_time_val.into());

    let duration_key = v8::String::new(scope, "duration").unwrap();
    let duration_val = v8::Number::new(scope, millis);
...
pub fn setup_performance_api(
    ...
    // performance.now()
    let now_name = v8::String::new(scope, "now").unwrap();
    let now_fn = v8::Function::new(scope, performance_now_callback).unwrap();
    performance.set(scope, now_name.into(), now_fn.into());

    // performance.timeOrigin (as property, not function)
    let time_origin_name = v8::String::new(scope, "timeOrigin").unwrap();
    ...
    let elapsed_since_start = now.duration_since(*start);
    let origin_millis = (system_now - elapsed_since_start).as_secs_f64() * 1000.0;
    let time_origin_val = v8::Number::new(scope, origin_millis);
    performance.set(scope, time_origin_name.into(), time_origin_val.into());

    // performance.mark()
    let mark_name = v8::String::new(scope, "mark").unwrap();
    let mark_fn = v8::Function::new(scope, performance_mark_callback).unwrap();
    performance.set(scope, mark_name.into(), mark_fn.into());

    // performance.measure()
    let measure_name = v8::String::new(scope, "measure").unwrap();
    let measure_fn = v8::Function::new(scope, performance_measure_callback).unwrap();
    performance.set(scope, measure_name.into(), measure_fn.into());

    // performance.getEntries()
    let get_entries_name = v8::String::new(scope, "getEntries").unwrap();
    let get_entries_fn = v8::Function::new(scope, performance_get_entries_callback).unwrap();
    performance.set(scope, get_entries_name.into(), get_entries_fn.into());

    // performance.getEntriesByType() - placeholder
    let get_by_type_name = v8::String::new(scope, "getEntriesByType").unwrap();
    let get_by_type_fn = v8::Function::new(scope, performance_get_entries_callback).unwrap();
    performance.set(scope, get_by_type_name.into(), get_by_type_fn.into());

    // performance.getEntriesByName() - placeholder
    let get_by_name_name = v8::String::new(scope, "getEntriesByName").unwrap();
    let get_by_name_fn = v8::Function::new(scope, performance_get_entries_callback).unwrap();
    performance.set(scope, get_by_name_name.into(), get_by_name_fn.into());

    // Bind to global
    let perf_name = v8::String::new(scope, "performance").unwrap();
