...
pub fn setup_timer_api(
    ...
    // Setup setTimeout
    let set_timeout_template = v8::FunctionTemplate::new(scope, set_timeout_callback);
    let set_timeout_func = set_timeout_template.get_function(scope).unwrap();
    let set_timeout_key = v8::String::new(scope, "setTimeout").unwrap();
    global.set(scope, set_timeout_key.into(), set_timeout_func.into());

    // Setup setInterval
    let set_interval_template = v8::FunctionTemplate::new(scope, set_interval_callback);
    let set_interval_func = set_interval_template.get_function(scope).unwrap();
    let set_interval_key = v8::String::new(scope, "setInterval").unwrap();
    global.set(scope, set_interval_key.into(), set_interval_func.into());

    // Setup clearTimeout
    let clear_timeout_template = v8::FunctionTemplate::new(scope, clear_timeout_callback);
    let clear_timeout_func = clear_timeout_template.get_function(scope).unwrap();
    ...
    // Setup clearInterval
    let clear_interval_template = v8::FunctionTemplate::new(scope, clear_interval_callback);
    let clear_interval_func = clear_interval_template.get_function(scope).unwrap();
    ...
    let queue_microtask_template = v8::FunctionTemplate::new(scope, queue_microtask_callback);
    let queue_microtask_func = queue_microtask_template.get_function(scope).unwrap();
...
fn set_timeout_callback(
    ...
    if delay_ms == 0 {
        ...
        let _ = callback_func.call(scope, undefined.into(), &[]);
    } else {
