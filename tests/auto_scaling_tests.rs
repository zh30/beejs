...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_queue_length_tracking() {
        ...
        let pool: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(ProcessPool::new(config))))))).expect("Failed to create pool"));
