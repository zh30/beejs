...
mod blob_api_tests {
    ...
    fn test_blob_constructor_string() {
        ...
        assert!(stdout.contains("blob.size: 13"), "Expected blob.size 13. Got: {}", stdout);
        assert!(stdout.contains("blob.type: text/plain"), "Expected blob.type text/plain. Got: {}", stdout);
        assert!(stdout.contains("blob instanceof Blob: true"), "Expected Blob instance. Got: {}", stdout);
    }
    ...
    fn test_blob_constructor_empty() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("empty blob size: 0"), "Expected empty blob size 0. Got: {}", stdout);
        assert!(stdout.contains("empty blob type: application/octet-stream"), "Expected octet-stream type. Got: {}", stdout);
    }
    ...
    fn test_blob_constructor_multiple_parts() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("multi part size: 10"), "Expected size 10 (Hello + space + World). Got: {}", stdout);
    }
    ...
    fn test_blob_text() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("blob text: Hello, Beejs!"), "Expected correct text. Got: {}", stdout);
        assert!(stdout.contains("text correct: true"), "Expected text to match. Got: {}", stdout);
    }
    ...
    fn test_blob_slice() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("sliced size: 5"), "Expected sliced size 5. Got: {}", stdout);
        assert!(stdout.contains("sliced text: Hello"), "Expected sliced text 'Hello'. Got: {}", stdout);
    }
    ...
    fn test_blob_slice_negative_start() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("slice from end size: 6"), "Expected size 6. Got: {}", stdout);
    }
    ...
    fn test_blob_slice_with_content_type() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("sliced with type: text/html"), "Expected content type override. Got: {}", stdout);
    }
    ...
    fn test_blob_stream() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("stream exists: true"), "Expected stream to exist. Got: {}", stdout);
        assert!(stdout.contains("stream.getReader exists: true"), "Expected getReader method. Got: {}", stdout);
    }
    ...
    fn test_file_constructor() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("file.name: test.txt"), "Expected file name test.txt. Got: {}", stdout);
        assert!(stdout.contains("file.size: 17"), "Expected file size 17. Got: {}", stdout);
        assert!(stdout.contains("file.type: text/plain"), "Expected file type text/plain. Got: {}", stdout);
        assert!(stdout.contains("file instanceof File: true"), "Expected File instance. Got: {}", stdout);
        assert!(stdout.contains("file instanceof Blob: true"), "Expected File to be Blob instance. Got: {}", stdout);
    }
    ...
    fn test_file_with_last_modified() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("file.name: doc.txt"), "Expected file name doc.txt. Got: {}", stdout);
    }
    ...
    fn test_blob_unicode() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("contains emoji: true"), "Expected emoji in text. Got: {}", stdout);
    }
    ...
    fn test_blob_binary() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("binary blob size: 7"), "Expected size 7. Got: {}", stdout);
        assert!(stdout.contains("contains null byte: true"), "Expected null byte preserved. Got: {}", stdout);
    }
    ...
    fn test_blob_methods_on_prototype() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("text is function: true"), "Expected text method. Got: {}", stdout);
        assert!(stdout.contains("slice is function: true"), "Expected slice method. Got: {}", stdout);
        assert!(stdout.contains("stream is function: true"), "Expected stream method. Got: {}", stdout);
        assert!(stdout.contains("arrayBuffer is function: true"), "Expected arrayBuffer method. Got: {}", stdout);
    }
    ...
    fn test_file_inherits_blob_methods() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("file.text method: true"), "Expected file.text method. Got: {}", stdout);
        assert!(stdout.contains("file.slice method: true"), "Expected file.slice method. Got: {}", stdout);
        assert!(stdout.contains("file.slice result type: true"), "Expected slice to return File. Got: {}", stdout);
    }
    ...
    fn test_blob_stream_readable() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("reader exists: true"), "Expected reader to exist. Got: {}", stdout);
        assert!(stdout.contains("reader.read exists: true"), "Expected read method. Got: {}", stdout);
    }
}
