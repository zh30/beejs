...
fn test_fs_module_exists() {
    ...
    assert_eq!(result.trim(), "object", "require('fs') should return an object");
}
...
fn test_readfilesync_returns_file_content() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert_eq!(result.trim(), "Hello, Beejs!", "readFileSync should return file content");
}
...
fn test_writefilesync_creates_file() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert!(result.trim().is_empty() || result.trim() == "undefined", "writeFileSync should return undefined");

    // Verify file was created
    let content = fs::read_to_string(&test_file).expect("Failed to read output file");
    assert_eq!(content, "Test content from Beejs!", "File should contain written content");
}
...
fn test_existssync_returns_true_for_existing_file() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "existsSync should return true for existing file");
}
...
fn test_existssync_returns_false_for_nonexistent_file() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert_eq!(result.trim(), "false", "existsSync should return false for non-existent file");
}
...
fn test_mkdirsync_creates_directory() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert!(result.trim().is_empty() || result.trim() == "undefined", "mkdirSync should return undefined");
...
fn test_readdirsync_returns_file_list() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert!(result.contains("file1.txt"), "readdirSync should return file names");
    assert!(result.contains("file2.txt"), "readdirSync should return file names");
    assert!(result.contains("file3.txt"), "readdirSync should return file names");
}
...
fn test_unlinksync_deletes_file() {
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert!(result.trim().is_empty() || result.trim() == "undefined", "unlinkSync should return undefined");
...
fn test_rmdirsync_removes_directory() {
    ...
    assert!(sub_dir.exists(), "Sub directory should exist before removal");
    ...
    let result = runtime.execute_code(&code).expect("Execution failed");
    assert!(result.trim().is_empty() || result.trim() == "undefined", "rmdirSync should return undefined");
...
fn test_fs_module_has_all_functions() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true,true,true,true,true,true,true", "All fs functions should exist");
}
