...
mod tests {
    ...
    fn test_module_loader_from_current_dir() {
        let loader = ModuleLoader::from_current_dir();
    ...
    fn test_resolve_relative_module_dot_slash() {
        ...
        // 创建测试模块文件
        let module_path = temp_dir.path().join("test_module.js");
        std::fs::write(&module_path, "module.exports = {};").unwrap();

        let result = loader.resolve_module("./test_module");
    ...
    fn test_resolve_builtin_modules() {
        ...
        let builtin_modules = vec!["path", "fs", "os", "crypto", "buffer"];
    ...
    fn test_module_cache() {
        ...
        // 创建测试模块
        let module_path = temp_dir.path().join("cached_module.js");
        std::fs::write(&module_path, "module.exports = { value: 42 };").unwrap();

        // 解析模块（缓存功能尚未完全实现）
        let result = loader.resolve_module("./cached_module");
    ...
    fn test_resolve_absolute_module() {
        ...
        let absolute_path = "/absolute/path/to/module.js";
        let result = loader.resolve_module(absolute_path);
    ...
    fn test_add_js_extension() {
        ...
        // 创建不带扩展名的模块文件
        let module_path = temp_dir.path().join("noext_module.js");
        std::fs::write(&module_path, "module.exports = {};").unwrap();

        let result = loader.resolve_module("./noext_module");
    ...
    fn test_resolve_node_modules() {
        ...
        let package_dir = node_modules_dir.join("test-package");
        ...
        // 创建主入口文件
        let main_file = package_dir.join("index.js");
        std::fs::write(&main_file, "module.exports = {};").unwrap();

        let result = loader.resolve_module("test-package");
    ...
    fn test_package_main_entry_point() {
        ...
        // 创建带自定义主入口的包
        let node_modules_dir = temp_dir.path().join("node_modules");
        let package_dir = node_modules_dir.join("custom-main");
        std::fs::create_dir_all(&package_dir).unwrap();
        ...
        let main_file = lib_dir.join("index.js");
        std::fs::write(&main_file, "module.exports = {};").unwrap();

        let result = loader.resolve_module("custom-main");
    ...
    fn test_invalid_package_json() {
        ...
        // 创建无效的 package.json
        let node_modules_dir = temp_dir.path().join("node_modules");
        let package_dir = node_modules_dir.join("invalid-pkg");
        std::fs::create_dir_all(&package_dir).unwrap();

        let package_json = package_dir.join("package.json");
        std::fs::write(&package_json, "{ invalid json }").unwrap();

        let result = loader.resolve_module("invalid-pkg");
        // 应该失败或使用默认行为
    ...
    fn test_circular_dependency_simulation() {
        ...
        // 创建两个相互引用的模块
        let module_a = temp_dir.path().join("module_a.js");
        let module_b = temp_dir.path().join("module_b.js");

        std::fs::write(&module_a, "require('./module_b'); module.exports = {};").unwrap();
        std::fs::write(&module_b, "require('./module_a'); module.exports = {};").unwrap();

        // 解析第一个模块
        let result = loader.resolve_module("./module_a");
    ...
    fn test_module_path_normalization() {
        ...
        // 创建带特殊字符的模块目录
        let special_dir = temp_dir.path().join("my-module_v1.0");
        std::fs::create_dir_all(&special_dir).unwrap();

        let index_path = special_dir.join("index.js");
        std::fs::write(&index_path, "module.exports = {};").unwrap();

        let result = loader.resolve_module("./my-module_v1.0");
