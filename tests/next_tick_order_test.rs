...
fn test_next_tick_before_timer() {
    ...
    assert_eq!(result.trim(), "nextTick,timer",
        "nextTick should execute before timer. Got: {}", result.trim());
}
...
fn test_next_tick_before_setimmediate() {
    ...
    assert_eq!(result.trim(), "nextTick,immediate",
        "nextTick should execute before setImmediate. Got: {}", result.trim());
}
...
fn test_multiple_next_ticks() {
    ...
    assert_eq!(result.trim(), "1,2,3",
        "Multiple nextTicks should execute in order. Got: {}", result.trim());
}
...
fn test_next_tick_chain() {
    ...
    assert_eq!(result.trim(), "a,b,c",
        "Chained nextTicks should execute in order (a, then b, then c). Got: {}", result.trim());
}
...
fn test_next_tick_error_handling() {
    ...
    "#).unwrap();
    assert!(result.trim().contains("callback must be a function"),
        "nextTick should throw for non-function. Got: {}", result.trim());
}
...
fn test_next_tick_returns_undefined() {
    ...
    assert_eq!(result.trim(), "true",
        "nextTick should return undefined. Got: {}", result.trim());
}
...
fn test_next_tick_with_promise() {
    ...
    assert!(result.trim() == "nextTick" || result.trim() == "promise,nextTick" || result.trim() == "nextTick,promise",
        "nextTick and Promise order should be predictable. Got: {}", result.trim());
}
