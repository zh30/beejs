...
mod performance_api_tests {
    ...
    fn test_performance_now_returns_number() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("number"), "performance.now should return a number");
    }
    ...
    fn test_performance_now_monotonic() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("true"), "performance.now should be monotonic");
    }
    ...
    fn test_performance_time_origin_exists() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("number"), "performance.timeOrigin should exist");
    }

    #[test]
    fn test_performance_time_origin_reasonable() {
        let output = Command::new(beejs_path())
            .args(["eval", "console.log(performance.timeOrigin > 1700000000000)"])
            .output()
            .expect("Failed to run beejs");

        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("true"), "timeOrigin should be a reasonable Unix timestamp");
    }
    ...
    fn test_performance_mark() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("1"), "performance.mark should create an entry");
    }
    ...
    fn test_performance_measure() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("1"), "performance.measure should create an entry");
        assert!(stdout.contains("measure"), "measure entry should have entryType 'measure'");
    }
    ...
    fn test_performance_measure_duration() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("true"), "measure should have positive duration");
    }
    ...
    fn test_performance_get_entries_by_type() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("1 1"), "getEntriesByType should filter by type");
    }
    ...
    fn test_performance_clear_marks() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("0"), "clearMarks should remove mark entries");
    }
    ...
    fn test_performance_clear_measures() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("0"), "clearMeasures should remove measure entries");
    }
    ...
    fn test_performance_to_json() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("object number number"), "toJSON should return an object with now and timeOrigin");
    }
    ...
    fn test_performance_measure_without_marks() {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.contains("1 true"), "measure should work without explicit marks (uses timeOrigin)");
    }
    ...
    fn test_performance_precision() {
        ...
        assert!(stdout.contains("number true"), "performance.now should return a number >= 0");
    }
