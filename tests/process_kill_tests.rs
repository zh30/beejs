...
fn test_process_kill_exists() {
    ...
    assert_eq!(result.trim(), "function", "process.kill should be a function");
}
...
fn test_process_kill_with_sigterm_number() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "process.kill with signal 15 should work");
}
...
fn test_process_kill_with_signal_number() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "process.kill should return false when killing self");
}
...
fn test_process_kill_with_invalid_pid() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "process.kill should handle invalid PID gracefully");
}
...
fn test_process_kill_with_sigint() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "process.kill should work with signal 2");
}
...
fn test_process_kill_with_sighup() {
    ...
    "#;
    let result = runtime.execute_code(code).expect("Execution failed");
    assert_eq!(result.trim(), "true", "process.kill should work with signal 1");
}
