...
mod string_fast_path_tests {
    ...
    fn test_string_split_fast_path() {
        ...
        // 测试字符串分割
        let result = runtime.execute_code(r#""a,b,c".split(",")"#).unwrap();
        assert!(result.contains("a") && result.contains("b") && result.contains("c"));

        let result = runtime.execute_code(r#""hello".split("")"#).unwrap();
        assert!(result.contains("h") && result.contains("e") && result.contains("l") && result.contains("o"));

        let result = runtime.execute_code(r#""one".split(",")"#).unwrap();
    ...
    fn test_string_method_with_variables() {
        ...
        // 变量应该回退到V8执行
        let result = runtime.execute_code(r#"let s = "hello"; s.length"#).unwrap();
        assert_eq!(result, "5");
    }
    ...
    fn test_string_method_complex_expressions() {
        ...
        // 复杂表达式应该回退到V8执行
        let result = runtime.execute_code(r#""hello" + "world".length"#).unwrap();
        assert_eq!(result, "hello5"); // "hello" + 5 (字符串连接)

        let result = runtime.execute_code(r#"("hello".length + "world".length).toString()"#).unwrap();
        assert_eq!(result, "10");
    }
