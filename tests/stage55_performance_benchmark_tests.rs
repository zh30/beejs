...
mod stage_55_performance_benchmark_tests {
    ...
    fn test_baseline_comparison() {
        ...
        // 设置基线
        let baseline = framework.run_benchmark(
            "test_baseline",
            MetricType::ExecutionTime,
            || 1,
        );
        ...
        // 比较结果
        let delta = framework.compare_with_baseline(&result);
    ...
    fn test_different_metric_types() {
        ...
        // 测试执行时间
        let exec_result = framework.run_benchmark(
            "test_exec_time",
            MetricType::ExecutionTime,
            || std::thread::sleep(Duration::from_millis(1)),
        );
        ...
        // 测试吞吐量
        let throughput_result = framework.run_benchmark(
            "test_throughput",
        ...
        // 测试内存使用
        let memory_result = framework.run_benchmark(
            "test_memory",
            MetricType::MemoryUsage,
            || vec![0; 1000],
        );
    ...
    fn test_benchmark_statistics() {
        ...
        let expected_min_total = result.avg_duration * result.iterations as u32;
