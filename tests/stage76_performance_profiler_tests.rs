...
mod tests {
    ...
    mod basic_profiler {
        ...
        fn test_concurrent_profiling() {
            ...
            for id in ids {
                let result = profiler.stop_profile(id).unwrap();
                results.push(result);
            }
        ...
        fn test_invalid_profile_operations() {
            ...
            // 尝试停止同一个 profile 两次
            let profile_id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
            let _ = profiler.stop_profile(profile_id);
            let result2 = profiler.stop_profile(profile_id);
    ...
    mod function_tracking {
        ...
        fn test_function_execution_time_tracking() {
            ...
            // 验证执行时间合理（不要求完全相等）
            let diff = if elapsed > result.execution_time {
        ...
        fn test_memory_tracking_during_execution() {
            ...
            let profile_id = profiler.start_profile(ProfileTarget::Memory).unwrap();
        ...
        fn test_nested_function_calls() {
            ...
            // 模拟嵌套调用
            let outer_id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
            std::thread::sleep(Duration::from_millis(10));

            let inner_id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
            std::thread::sleep(Duration::from_millis(5));

            let inner_result = profiler.stop_profile(inner_id).unwrap();
            assert!(inner_result.execution_time >= Duration::from_millis(5));

            let outer_result = profiler.stop_profile(outer_id).unwrap();
    ...
    mod hotspot_analysis {
        ...
        fn test_hotspot_identification() {
            ...
            for &target in &targets {
                let id = profiler.start_profile(target).unwrap();
    ...
    mod concurrent_profiling {
            ...
            let handles: Vec<std::thread::JoinHandle<ProfileResult>> = (0..10)
                .map(|i| {
                    std::thread::spawn(move || {
                        let mut profiler = Profiler::new(ProfilingMode::Detailed).unwrap();
                        let id = profiler.start_profile(ProfileTarget::Concurrent).unwrap();
                        std::thread::sleep(Duration::from_millis(i));
                        profiler.stop_profile(id).unwrap()
                    })
                })
        ...
        fn test_concurrent_statistics_accuracy() {
            let profiler = Profiler::new(ProfilingMode::Basic).unwrap();
    ...
    mod edge_cases {
        ...
        fn test_profiler_reset() {
            ...
            let stats1 = profiler.get_statistics().clone();

            // 验证初始统计
            assert_eq!(stats1.total_profiles, 5);

            // 创建新的 profiler（模拟重置）
            let new_profiler = Profiler::new(ProfilingMode::Basic).unwrap();
            let stats2 = new_profiler.get_statistics();
    ...
    mod integration_tests {
        ...
        fn test_real_world_scenario() {
            ...
            // 模拟真实的应用场景
            // 1. 初始化
            let init_id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
            std::thread::sleep(Duration::from_millis(5));
            let _init_result = profiler.stop_profile(init_id);

            // 2. 数据处理
            let data_id = profiler.start_profile(ProfileTarget::Memory).unwrap();
            let mut data = Vec::new();
            for i in 0..1000 {
                data.push(i * 2);
            }
            let _data_result = profiler.stop_profile(data_id);

            // 3. 算法执行
            let algo_id = profiler.start_profile(ProfileTarget::Jit).unwrap();
            let _result = data.iter().sum::<i32>();
            let _algo_result = profiler.stop_profile(algo_id);

            // 4. 清理
            let cleanup_id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
            data.clear();
            std::thread::sleep(Duration::from_millis(2));
            let _cleanup_result = profiler.stop_profile(cleanup_id);
        ...
        fn test_comparison_benchmark() {
            ...
            // 相同的 workload
            let workload = |profiler: &mut Profiler| {
                for _ in 0..100 {
                    let id = profiler.start_profile(ProfileTarget::Runtime).unwrap();
                    std::thread::sleep(Duration::from_millis(1));
                    profiler.stop_profile(id).unwrap();
