...
mod stage78_phase1_integration_tests {
    ...
    fn test_simd_auto_vectorization_integration() {
        ...
        let optimized_small = engine.auto_vectorize(&small_data);
        ...
        let optimized_large = engine.auto_vectorize(&large_data);
        assert_eq!(optimized_large.len(), large_data.len());
        println!("   ✓ 大数据集自动优化: {} elements", optimized_large.len());

        // 测试循环向量化
        let loop_result = engine.auto_vectorize_loop(1000, 0.0, 1.0);
    ...
    fn test_simd_data_layout_optimization() {
        ...
        // 应用数据布局优化
        let optimized = engine.optimize_data_layout(&data);
    ...
    fn test_simd_batch_reduction_integration() {
        ...
        // 执行批处理归约
        let results = engine.batch_reduce(&batch_data);
    ...
    fn test_multithreaded_simd_integration() {
        ...
        let thread_manager = WasmThreadsManager::new(config);
        ...
        let handles: Vec<_> = (0..num_tasks).map(|task_id| {
            let simd_clone = Arc::clone(&simd_engine);
            thread_manager.spawn(move || {
                ...
                // 执行 SIMD 向量加法
                let result = simd_clone.vector_add_f32(&data, &data);

                // 执行 SIMD 点积
                let dot_product = simd_clone.dot_product_f32(&data, &data);
    ...
    fn test_simd_with_shared_memory_integration() {
        ...
        let simd_engine = SimdEngine::new();
        let config = ThreadPoolConfig::default();
        let thread_manager = WasmThreadsManager::new(config);

        // 创建共享内存
        let shared_mem = thread_manager.create_shared_memory(4096).expect("共享内存创建失败");
        ...
        // 启动工作线程执行 SIMD 操作
        let handle = thread_manager.spawn(move || {
            // 读取共享内存中的数据
            ...
            // 执行 SIMD 操作
            let sum = simd_engine.vector_sum_f32(shared_data);
            let max = simd_engine.vector_max_f32(shared_data);
    ...
    fn test_comprehensive_performance_integration() {
        ...
        // 2. 并行归约
        let reduce_results = simd_engine.batch_reduce(&batch_results);

        // 3. 多线程 SIMD 操作
        let handles: Vec<_> = (0..4).map(|i| {
            let simd_clone = Arc::new(SimdEngine::new());
        ...
        // 计算吞吐量
        let total_elements = num_batches * batch_size * 2; // 批处理涉及两个批次
        let throughput = total_elements as f64 / total_time.as_millis() as f64 * 1000.0;
    ...
    fn test_phase1_complete_integration() {
        ...
        println!("   验证线程管理器功能...");
        let thread_manager = WasmThreadsManager::new(ThreadPoolConfig::default());
        ...
        let optimized = simd_engine.auto_vectorize(&data);
        assert_eq!(optimized.len(), data.len());

        println!("   验证批处理功能...");
        let batches = vec![vec![1.0, 2.0], vec![3.0, 4.0]];
        let results = simd_engine.batch_reduce(&batches);
        ...
        let sum = handle.join().expect("任务执行失败");
        assert!((sum - 15.0).abs() < f32::EPSILON);

        // 验证统计数据
        let stats = simd_engine.get_stats();
        println!("   最终统计:");
