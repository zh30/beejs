...
fn test_error_handling() {
    ...
    // 测试语法错误
    let result = runtime.execute_standard("invalid syntax @#$%");
    assert!(result.is_err(), "语法错误应该返回 Err");

    // 测试运行时错误
    let result = runtime.execute_standard("throw new Error('test error')");
...
fn test_boundary_values() {
    ...
    // 测试最大安全整数
    let result = runtime.execute_standard("Number.MAX_SAFE_INTEGER");
    assert!(result.is_ok());
    assert!(result.unwrap().trim() == "9007199254740991");

    // 测试无穷大
    let result = runtime.execute_standard("Infinity");
    assert!(result.is_ok());

    // 测试 NaN
    let result = runtime.execute_standard("NaN");
    assert!(result.is_ok());
...
fn test_large_string() {
    ...
    let code = format!("'{}'.length", large_string);
...
fn test_large_array() {
    ...
    // 创建包含 10000 个元素的数组
    let code = "Array(10000).fill(0).length";
    let result = runtime.execute_standard(code);
...
fn test_long_running_stability() {
    ...
    for i in 0..iterations {
        let code = format!("let sum = 0; for(let i = 0; i < 100; i++) {{ sum += i; }} sum");
        let result = runtime.execute_standard(&code);
...
fn test_error_recovery() {
    ...
    // 在错误后尝试正常执行
    let _ = runtime.execute_standard("throw new Error('first error')");
    let result = runtime.execute_standard("1 + 1");
...
fn test_resource_cleanup() {
    ...
    for _ in 0..10 {
        let runtime = RuntimeLite::new(false).expect("Failed to create runtime");
        let result = runtime.execute_standard("'test'");
        assert!(result.is_ok());
        // runtime 在作用域结束时自动销毁
    }
...
fn test_runtime_configuration() {
    ...
    // 验证基本功能
    let result = runtime.execute_standard("typeof console");
    assert!(result.is_ok());
...
fn test_concurrent_aware_isolation() {
    ...
    let result1 = runtime.execute_standard("let x = 42; x");
    let result2 = runtime.execute_standard("typeof x");
