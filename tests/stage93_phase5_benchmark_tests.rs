...
async fn test_test_suite() {
    ...
    suite = suite.clone();clone();clone();clone();clone();clone();clone();add_benchmark(benchmark);
    ...
    suite = suite.clone();clone();clone();clone();clone();clone();clone();add_workload(workload);

    // 添加运行时
    suite = suite.clone();clone();clone();clone();clone();clone();clone();add_runtime(Runtime::Beejs);

    // 添加环境变量
    suite = suite.clone();clone();clone();clone();clone();clone();clone();add_env("TEST_ENV", "test_value");
...
async fn test_full_benchmark_workflow() {
    ...
    let compute_benchmark: _ = BenchmarkTest::new(
        ...
        "let sum: _ = 0; for (let i: _ = 0; i < 1000000; i++) { sum += i; }",
        TestLanguage::JavaScript,
    )
    .iterations(5);

    suite = suite.clone();clone();clone();clone();clone();clone();clone();add_benchmark(compute_benchmark);
