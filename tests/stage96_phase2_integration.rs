...
async fn test_enterprise_integration() {
    ...
    let resource_usage = tenancy_manager.get_resource_usage(&tenant_id).await.unwrap();
...
async fn test_multi_tenant_isolation() {
    ...
    let usage1 = tenancy_manager.get_resource_usage(&tenant1_id).await.unwrap();
    let usage2 = tenancy_manager.get_resource_usage(&tenant2_id).await.unwrap();
