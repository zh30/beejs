...
mod tests {
    ...
    async fn test_15000_concurrent_scripts() {
        ...
        for i in 0..concurrent_scripts {
            ...
            let script = format!("const result = {} + {}; result;", i % 100, i % 50);
    ...
    async fn test_smart_scheduling_optimization() {
        ...
        for i in 0..tasks_per_type {
            let script = format!("const x = {}; const y = {}; x + y;", i, i % 10);
            let pool_clone = Arc::clone(&pool);
            let handle = task::spawn(async move {
                if let Some(runtime) = pool_clone.get_runtime() {
                    let result = runtime.execute_code(&script);
    ...
    async fn test_work_stealing_optimization() {
        ...
        for worker_id in 0..pool_size {
            ...
            for i in 0..task_count {
                let script = format!("let sum = 0; for (let j = 0; j < 10; j++) {{ sum += {} * j; }} sum;", i);
                let pool_clone = Arc::clone(&pool);
    ...
    async fn test_memory_sharing_optimization() {
        ...
        let shared_regions = 50;
        let operations_per_region = 20; // Reduced for testing
        let mut config = ConcurrentConfig::default();
        config.enable_prewarm = false;
        let pool = Arc::new(ConcurrentRuntimePool::new(config));
    ...
    async fn test_shared_object_cache_effectiveness() {
        ...
        for i in 0..100 {
            let value = format!("common_string_{}", i);
        ...
        for i in 0..1000 {
            ...
            let handle = task::spawn(async move {
                ...
                for j in 0..10 {
                    ...
                    let _ = interner_clone.intern(&value);
                }
                // Add some new strings
                let new_value = format!("dynamic_{}", i);
                let _ = interner_clone.intern(&new_value);
            });
            handles.push(handle);
        }
    ...
    async fn test_load_balancing_efficiency() {
        ...
        for i in 0..total_tasks {
            let script = format!("let result = 0; for (let j = 0; j < 5; j++) {{ result += {} * j; }} result;", i);
            let pool_clone = Arc::clone(&pool);
            let _ = task::spawn(async move {
                if let Some(runtime) = pool_clone.get_runtime() {
                    let _ = runtime.execute_code(&script);
    ...
    async fn test_performance_regression_validation() {
        ...
        for iteration in 0..test_iterations {
            ...
            for i in 0..tasks_per_iteration {
                let script = format!("const result = Math.sqrt({}); result;", i);
                let pool_clone = Arc::clone(&pool);
                let handle = task::spawn(async move {
                    if let Some(runtime) = pool_clone.get_runtime() {
                        let result = runtime.execute_code(&script);
        ...
        if iteration_times.len() >= 2 {
            let avg_time = iteration_times.iter().sum::<Duration>().as_millis() as f64 / iteration_times.len() as f64;
            let first_time = iteration_times[0].as_millis() as f64;
            let variance = ((avg_time - first_time) / first_time).abs();
    ...
    async fn test_end_to_end_concurrent_performance() {
        ...
        for i in 0..5000 {
            let script = format!(r#"
                ...
                avg;
            "#, i % 100);
            let pool_clone = Arc::clone(&pool2);
            let handle = task::spawn(async move {
                if let Some(runtime) = pool_clone.get_runtime() {
                    let result = runtime.execute_code(&script);
                    pool_clone.return_runtime(runtime);
        ...
        for i in 0..2000 {
            let script = format!(r#"
                ...
                fib({} % 15);
            "#, i);
            let pool_clone = Arc::clone(&pool3);
            let handle = task::spawn(async move {
                if let Some(runtime) = pool_clone.get_runtime() {
                    let result = runtime.execute_code(&script);
                    pool_clone.return_runtime(runtime);
