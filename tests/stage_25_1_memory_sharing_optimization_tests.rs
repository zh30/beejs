...
mod tests {
    ...
    async fn test_create_cow_shared_memory() {
        ...
        // 写入初始数据
        let initial_data = b"Initial shared data for COW test";
        manager.write(&mut handle, 0, initial_data).unwrap();

        // 读取验证
        let read_data = manager.read(&handle, 0, initial_data.len()).unwrap();
    ...
    async fn test_copy_on_write_mechanism() {
        ...
        // 写入原始数据
        let original_data = b"Original data - should be shared";
        ...
        // 验证两个句柄看到相同的数据
        let data1 = manager.read(&handle1, 0, original_data.len()).unwrap();
        let data2 = manager.read(&handle2, 0, original_data.len()).unwrap();
        assert_eq!(data1, data2);
        assert_eq!(data1, original_data);

        // 通过句柄2修改数据（自动创建COW副本）
        let modified_data = b"Modified data - should copy";
        manager.write(&mut handle2, 0, modified_data).unwrap();

        // 句柄2应该看到修改后的数据（来自COW副本）
        let data2_modified = manager.read(&handle2, 0, modified_data.len()).unwrap();
        assert_eq!(data2_modified, modified_data);

        // 句柄1应该仍然看到原始数据（独立副本）
        let data1_unchanged = manager.read(&handle1, 0, original_data.len()).unwrap();
    ...
    async fn test_memory_prefetch() {
        ...
        // 写入测试数据到不同位置
        let chunk1 = b"Chunk 1 data for prefetch test";
        let chunk2 = b"Chunk 2 data for prefetch test";
        let chunk3 = b"Chunk 3 data for prefetch test";
        ...
        // 验证数据正确性
        let verify1 = manager.read(&handle, 0, chunk1.len()).unwrap();
        let verify2 = manager.read(&handle, 2048, chunk2.len()).unwrap();
        let verify3 = manager.read(&handle, 4096, chunk3.len()).unwrap();
    ...
    async fn test_cow_performance() {
        ...
        let large_data = vec![42u8; 8192]; // 8KB 数据
    ...
    async fn test_shared_memory_stats() {
        ...
        for i in 0..5 {
            ...
            // 读取验证
            let _ = manager.read(&handle, 0, data.len()).unwrap();
        }
    ...
    async fn test_large_file_cow_mapping() {
        ...
        for chunk_idx in 0..10 { // 验证前10个块
            let offset = chunk_idx * chunk_size;
            let data = manager.read(&handle, offset, chunk_size).unwrap();
            let expected = vec![(chunk_idx % 256) as u8; chunk_size];
    ...
    async fn test_memory_prefetch_cache_effect() {
        ...
        // 填充数据
        let test_string = b"Cache test data chunk 0".to_vec();
        let test_len = test_string.len();
        println!("Test string length: {}", test_len);

        for i in 0..100 {
            let offset = i * 256;
            let data = format!("Cache test data chunk {}", i);
        ...
        // 验证数据正确性（使用正确的长度）
        let verify_data = manager.read(&handle, 0, test_len).unwrap();
