...
use std::collections::{HashMap, BTreeMap};
    ...
    async fn test_concurrent_cache_access() {
        let cache: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(EdgeCache::new("l1", 1000))))))).unwrap());
        let mut handles = vec![];
