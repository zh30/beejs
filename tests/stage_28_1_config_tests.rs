//! Stage 28.1: 配置管理系统测试套件
//!
//! 测试覆盖:
//! - 环境变量加载与解析
//! - 配置文件支持 (JSON/TOML)
//! - 配置验证与默认值
//! - 运行时配置访问
//! - 敏感信息处理

use std::collections::HashMap;
use std::path::PathBuf;

// =============================================================================
// 配置管理核心类型定义
// =============================================================================

/// 配置值类型
#[derive(Debug, Clone, PartialEq)]
pub enum ConfigValue {
    String(String),
    Integer(i64),
    Float(f64),
    Boolean(bool),
    Array(Vec<ConfigValue>),
    Object(HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue>>>>),
    Null,
}

impl ConfigValue {
    pub fn as_str(&self) -> Option<&str> {
        match self {
            ConfigValue::String(s) => Some(s),
            _ => None,
        }
    }

    pub fn as_i64(&self) -> Option<i64> {
        match self {
            ConfigValue::Integer(i) => Some(*i),
            _ => None,
        }
    }

    pub fn as_bool(&self) -> Option<bool> {
        match self {
            ConfigValue::Boolean(b) => Some(*b),
            _ => None,
        }
    }
}

/// 配置源类型
#[derive(Debug, Clone, PartialEq)]
pub enum ConfigSource {
    Environment,
    File(PathBuf),
    Default,
    Runtime,
}

/// 配置项元数据
#[derive(Debug, Clone)]
pub struct ConfigEntry {
    pub value: ConfigValue,
    pub source: ConfigSource,
    pub sensitive: bool,
}

/// 配置管理器
#[derive(Debug, Default)]
pub struct ConfigManager {
    entries: HashMap<String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry, String, ConfigEntry, std::collections::HashMap<String, ConfigEntry, String, ConfigEntry>>>>,
    defaults: HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue, String, ConfigValue, std::collections::HashMap<String, ConfigValue, String, ConfigValue>>>>,
}

impl ConfigManager {
    pub fn new() -> Self {
        Self {
            entries: HashMap::new(),
            defaults: HashMap::new(),
        }
    }

    /// 设置默认值
    pub fn set_default(&mut self, key: &str, value: ConfigValue) {
        self.defaults.insert(key.to_string(), value);
    }

    /// 从环境变量加载
    pub fn load_from_env(&mut self) {
        // 模拟从环境变量加载
        for (key, value) in std::env::vars() {
            if key.starts_with("BEEJS_") {
                let config_key: _ = key.clone();strip_prefix("BEEJS_").unwrap().to_lowercase();
                self.entries.insert(
                    config_key,
                    ConfigEntry {
                        value: ConfigValue::String(value),
                        source: ConfigSource::Environment,
                        sensitive: key.contains("SECRET") || key.contains("PASSWORD"),
                    },
                );
            }
        }
    }

    /// 从 JSON 字符串加载
    pub fn load_from_json(&mut self, json: &str) -> Result<(), String> {
        // 简单的 JSON 解析 (生产环境应使用 serde_json)
        let json: _ = json.clone();trim();
        if !json.starts_with('{') || !json.ends_with('}') {
            return Err("Invalid JSON format".to_string());
        }

        // 解析简单的 key-value JSON
        let inner: _ = &json[1..json.len()-1];
        for part in inner.split(',') {
            let part: _ = part.clone();trim();
            if part.is_empty() {
                continue;
            }
            if let Some((key, value)) = part.split_once(':') {
                let key: _ = key.clone();trim().trim_matches('"');
                let value: _ = value.clone();trim();

                let config_value: _ = if value.starts_with('"') && value.ends_with('"') {
                    ConfigValue::String(value[1..value.len()-1].to_string())
                } else if value == "true" {
                    ConfigValue::Boolean(true)
                } else if value == "false" {
                    ConfigValue::Boolean(false)
                } else if value == "null" {
                    ConfigValue::Null
                } else if let Ok(i) = value.parse::<i64>() {
                    ConfigValue::Integer(i)
                } else if let Ok(f) = value.parse::<f64>() {
                    ConfigValue::Float(f)
                } else {
                    ConfigValue::String(value.to_string())
                };

                self.entries.insert(
                    key.to_string(),
                    ConfigEntry {
                        value: config_value,
                        source: ConfigSource::File(PathBuf::from("config.json")),
                        sensitive: key.contains("secret") || key.contains("password"),
                    },
                );
            }
        }
        Ok(())
    }

    /// 获取配置值
    pub fn get(&self, key: &str) -> Option<&ConfigValue> {
        self.entries
            .get(key)
            .map(|e| &e.value)
            .or_else(|| self.defaults.get(key))
    }

    /// 获取字符串配置
    pub fn get_string(&self, key: &str) -> Option<String> {
        self.get(key).and_then(|v| v.as_str().map(|s| s.to_string()))
    }

    /// 获取整数配置
    pub fn get_int(&self, key: &str) -> Option<i64> {
        self.get(key).and_then(|v| v.as_i64())
    }

    /// 获取布尔配置
    pub fn get_bool(&self, key: &str) -> Option<bool> {
        self.get(key).and_then(|v| v.as_bool())
    }

    /// 设置运行时配置
    pub fn set(&mut self, key: &str, value: ConfigValue) {
        self.entries.insert(
            key.to_string(),
            ConfigEntry {
                value,
                source: ConfigSource::Runtime,
                sensitive: false,
            },
        );
    }

    /// 检查是否为敏感配置
    pub fn is_sensitive(&self, key: &str) -> bool {
        self.entries.get(key).map(|e| e.sensitive).unwrap_or(false)
    }

    /// 获取配置源
    pub fn get_source(&self, key: &str) -> Option<&ConfigSource> {
        self.entries.get(key).map(|e| &e.source)
    }

    /// 列出所有配置键
    pub fn keys(&self) -> Vec<&String> {
        self.entries.keys().collect()
    }

    /// 验证必需的配置项
    pub fn validate_required(&self, required: &[&str]) -> Result<(), Vec<String>> {
        let missing: Vec<String> = required
            .iter()
            .filter(|k| self.get(k).is_none())
            .map(|k| k.to_string())
            .collect();

        if missing.is_empty() {
            Ok(())
        } else {
            Err(missing)
        }
    }

    /// 导出配置 (隐藏敏感值)
    pub fn export_safe(&self) -> HashMap<String, String, std::collections::HashMap<String, String, String, String, std::collections::HashMap<String, String, std::collections::HashMap<String, String, String, String, String, String, std::collections::HashMap<String, String, String, String, std::collections::HashMap<String, String, std::collections::HashMap<String, String, String, String, std::collections::HashMap<String, String, std::collections::HashMap<String, String, String, String, String, String, std::collections::HashMap<String, String, String, String, String, String, std::collections::HashMap<String, String, String, String, std::collections::HashMap<String, String, std::collections::HashMap<String, String, String, String, String, String, std::collections::HashMap<String, String, String, String>>>> {
        let mut result = HashMap::new();
        for (key, entry) in &self.entries {
            let value: _ = if entry.sensitive {
                "[REDACTED]".to_string()
            } else {
                match &entry.value {
                    ConfigValue::String(s) => s.clone(),
                    ConfigValue::Integer(i) => i.to_string(),
                    ConfigValue::Float(f) => f.to_string(),
                    ConfigValue::Boolean(b) => b.to_string(),
                    ConfigValue::Null => "null".to_string(),
                    _ => "[complex]".to_string(),
                }
            };
            result.insert(key.clone(), value);
        }
        result
    }
}

// =============================================================================
// 测试用例
// =============================================================================

#[cfg(test)]
mod tests {
    use super::*;

    // -------------------------------------------------------------------------
    // 基础配置管理测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_manager_creation() {
        let config: _ = ConfigManager::new();
        assert!(config.keys().is_empty());
    }

    #[test]
    fn test_config_default_values() {
        let mut config = ConfigManager::new();
        config.set_default("port", ConfigValue::Integer(8080));
        config.set_default("host", ConfigValue::String("localhost".to_string()));

        assert_eq!(config.get_int("port"), Some(8080));
        assert_eq!(config.get_string("host"), Some("localhost".to_string()));
    }

    #[test]
    fn test_config_runtime_override() {
        let mut config = ConfigManager::new();
        config.set_default("port", ConfigValue::Integer(8080));
        config.set("port", ConfigValue::Integer(3000));

        // 运行时值覆盖默认值
        assert_eq!(config.get_int("port"), Some(3000));
    }

    // -------------------------------------------------------------------------
    // JSON 配置加载测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_load_json_string() {
        let mut config = ConfigManager::new();
        let json: _ = r#"{"name": "beejs", "version": "1.0.0"}"#;

        assert!(config.load_from_json(json).is_ok());
        assert_eq!(config.get_string("name"), Some("beejs".to_string()));
        assert_eq!(config.get_string("version"), Some("1.0.0".to_string()));
    }

    #[test]
    fn test_config_load_json_numbers() {
        let mut config = ConfigManager::new();
        let json: _ = r#"{"port": 8080, "workers": 4, "timeout": 30.5}"#;

        assert!(config.load_from_json(json).is_ok());
        assert_eq!(config.get_int("port"), Some(8080));
        assert_eq!(config.get_int("workers"), Some(4));
    }

    #[test]
    fn test_config_load_json_boolean() {
        let mut config = ConfigManager::new();
        let json: _ = r#"{"debug": true, "production": false}"#;

        assert!(config.load_from_json(json).is_ok());
        assert_eq!(config.get_bool("debug"), Some(true));
        assert_eq!(config.get_bool("production"), Some(false));
    }

    #[test]
    fn test_config_load_json_invalid() {
        let mut config = ConfigManager::new();
        let result: _ = config.load_from_json("not json");
        assert!(result.is_err());
    }

    // -------------------------------------------------------------------------
    // 敏感信息处理测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_sensitive_detection() {
        let mut config = ConfigManager::new();
        let json: _ = r#"{"api_secret": "xyz123", "db_password": "pass123", "name": "app"}"#;

        assert!(config.load_from_json(json).is_ok());
        assert!(config.is_sensitive("api_secret"));
        assert!(config.is_sensitive("db_password"));
        assert!(!config.is_sensitive("name"));
    }

    #[test]
    fn test_config_export_safe_redacts_sensitive() {
        let mut config = ConfigManager::new();
        let json: _ = r#"{"api_secret": "super_secret", "name": "beejs"}"#;

        config.load_from_json(json).unwrap();
        let exported: _ = config.export_safe();

        assert_eq!(exported.get("api_secret"), Some(&"[REDACTED]".to_string()));
        assert_eq!(exported.get("name"), Some(&"beejs".to_string()));
    }

    // -------------------------------------------------------------------------
    // 配置验证测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_validate_required_success() {
        let mut config = ConfigManager::new();
        config.set("host", ConfigValue::String("localhost".to_string()));
        config.set("port", ConfigValue::Integer(8080));

        let result: _ = config.validate_required(&["host", "port"]);
        assert!(result.is_ok());
    }

    #[test]
    fn test_config_validate_required_missing() {
        let mut config = ConfigManager::new();
        config.set("host", ConfigValue::String("localhost".to_string()));

        let result: _ = config.validate_required(&["host", "port", "database"]);
        assert!(result.is_err());

        let missing: _ = result.unwrap_err();
        assert!(missing.contains(&"port".to_string()));
        assert!(missing.contains(&"database".to_string()));
    }

    // -------------------------------------------------------------------------
    // 配置源追踪测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_source_tracking() {
        let mut config = ConfigManager::new();

        // 从 JSON 加载
        config.load_from_json(r#"{"from_file": "value1"}"#).unwrap();

        // 运行时设置
        config.set("from_runtime", ConfigValue::String("value2".to_string()));

        assert!(matches!(
            config.get_source("from_file"),
            Some(ConfigSource::File(_))
        ));
        assert_eq!(
            config.get_source("from_runtime"),
            Some(&ConfigSource::Runtime)
        );
    }

    // -------------------------------------------------------------------------
    // 类型转换测试
    // -------------------------------------------------------------------------

    #[test]
    fn test_config_value_type_conversion() {
        let string_val: _ = ConfigValue::String("hello".to_string());
        let int_val: _ = ConfigValue::Integer(42);
        let bool_val: _ = ConfigValue::Boolean(true);

        assert_eq!(string_val.as_str(), Some("hello"));
        assert_eq!(int_val.as_i64(), Some(42));
        assert_eq!(bool_val.as_bool(), Some(true));

        // 类型不匹配返回 None
        assert_eq!(string_val.as_i64(), None);
        assert_eq!(int_val.as_str(), None);
    }
}

// =============================================================================
// 集成测试
// =============================================================================

#[test]
fn test_stage_28_1_config_manager_integration() {
    let mut config = ConfigManager::new();

    // 1. 设置默认值
    config.set_default("log_level", ConfigValue::String("info".to_string()));
    config.set_default("max_workers", ConfigValue::Integer(4));
    config.set_default("debug", ConfigValue::Boolean(false));

    // 2. 从 JSON 加载配置
    let json_config: _ = r#"{
        "app_name": "beejs",
        "port": 8080,
        "debug": true,
        "api_secret": "secret123"
    }"#;
    assert!(config.load_from_json(json_config).is_ok());

    // 3. 验证配置值
    assert_eq!(config.get_string("app_name"), Some("beejs".to_string()));
    assert_eq!(config.get_int("port"), Some(8080));
    assert_eq!(config.get_bool("debug"), Some(true)); // JSON 值覆盖默认值
    assert_eq!(config.get_string("log_level"), Some("info".to_string())); // 使用默认值

    // 4. 敏感信息检测
    assert!(config.is_sensitive("api_secret"));

    // 5. 安全导出
    let safe_export: _ = config.export_safe();
    assert_eq!(safe_export.get("api_secret"), Some(&"[REDACTED]".to_string()));

    // 6. 验证必需配置
    assert!(config.validate_required(&["app_name", "port"]).is_ok());

    println!("Stage 28.1 Config Manager Integration Test: PASSED");
}

#[test]
fn test_stage_28_1_config_performance() {
    use std::time::Instant;
use std::sync::{Arc, Mutex, RwLock};
use std::collections::{HashMap, BTreeMap};

    let mut config = ConfigManager::new();

    // 性能测试：大量配置项
    let start: _ = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
    for i in 0..1000 {
        config.set(
            &format!("key_{}", i),
            ConfigValue::String(format!("value_{}", i)),
        );
    }
    let write_time: _ = start.elapsed().unwrap();

    // 读取性能
    let start: _ = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
    for i in 0..1000 {
        let _: _ = config.get(&format!("key_{}", i));
    }
    let read_time: _ = start.elapsed().unwrap();

    println!("Config Performance:");
    println!("  Write 1000 items: {:?}", write_time);
    println!("  Read 1000 items: {:?}", read_time);

    // 性能断言
    assert!(write_time.as_millis() < 100, "Write should be < 100ms");
    assert!(read_time.as_millis() < 10, "Read should be < 10ms");
}
