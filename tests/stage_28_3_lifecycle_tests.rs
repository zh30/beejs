...
impl HealthManager {
    pub fn new() -> Self {
        Self {
            checks: Vec::new(),
            ready: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicBool::new(false)))),
        }
    }
...
impl GracefulShutdown {
    pub fn new() -> Self {
        Self {
            ...
            active_connections: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicU32::new(0)))),
            shutdown_requested: Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicBool::new(false)))),
...
mod tests {
    ...
    fn test_graceful_shutdown_hooks() {
        ...
        let pre_called: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicBool::new(false))));
        let post_called: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicBool::new(false))));
...
use std::collections::{HashMap, BTreeMap};
    ...
    let cleanup_counter: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(AtomicU32::new(0))));
