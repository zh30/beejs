...
mod consistent_hashing_tests {
    ...
    fn test_minimal_key_migration() {
        ...
        // 理论上只有约 1/3 的键会迁移
        let migration_rate = migrated as f64 / original_routes.len() as f64;
...
mod intelligent_routing_tests {
    ...
    fn test_load_based_routing() {
        ...
        // 路由应该选择负载最低的节点
        let selected = router.route("test-key");
    ...
    fn test_sticky_routing() {
        ...
        let session_id = "session:user123";
        let first_route = router.route(session_id);

        for _ in 0..10 {
            let route = router.route(session_id);
...
mod circuit_breaker_tests {
    ...
    fn test_circuit_breaker_statistics() {
        ...
        let stats = breaker.get_statistics();
...
mod load_balancer_integration_tests {
    ...
    async fn test_load_balancer_request_routing() {
        ...
        // 路由请求
        let result = balancer.route_request(&request).await;
        assert!(result.is_ok());
    ...
    async fn test_load_balancer_failover() {
        ...
        // 所有请求应该路由到 backend-2
        let request = Request::new("req-1", "test-key");
        for _ in 0..5 {
            let result = balancer.route_request(&request).await;
            assert!(result.is_ok());
    ...
    async fn test_load_balancer_monitoring() {
        ...
        // 模拟请求
        for i in 0..100 {
            let request = Request::new(&format!("req-{}", i), &format!("key-{}", i));
            let _ = balancer.route_request(&request).await;
        }
