...
async fn test_fault_detector_creation() {
    ...
    let node_manager: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(NodeManager::new(service_discovery.clone())))));
    let _health_monitor: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(std::sync::Mutex::new(HealthMonitor::new(node_manager.clone())))));
