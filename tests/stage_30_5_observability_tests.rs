...
#[tokio::test]
async fn test_prometheus_gather_metrics() -> Result<(), Box<dyn std::error::Error>> {
    let mut exporter = PrometheusExporter::new().unwrap();

    // Add a test counter
    let counter_opts = Opts::new("test_counter_total", "Test counter");
    let counter = Counter::with_opts(counter_opts)?;
    counter.inc();
    exporter.registry().register(Box::new(counter.clone()))?;

    // Add a test gauge
    let gauge_opts = Opts::new("test_gauge", "Test gauge");
    let gauge = Gauge::with_opts(gauge_opts)?;
    gauge.set(42.0);
    exporter.registry().register(Box::new(gauge.clone()))?;

    // Gather metrics
    let metrics = exporter.gather_metrics()?;
    assert!(!metrics.is_empty());

    assert!(metrics.contains("test_counter_total"));
...
async fn test_alerting_system_creation() {
    ...
    let rules = system.rules.read().await;
...
#[tokio::test]
async fn test_observable_system_creation() -> Result<(), Box<dyn std::error::Error>> {
    let config = ObservabilityConfig::default();
    let system: ObservableSystem = ObservableSystem::new(config).await?;
    assert!(system.prometheus_exporter().is_some() || system.logger().is_some());
    Ok(())
}

/// Test observable system with metrics
#[tokio::test]
async fn test_observable_system_with_metrics() -> Result<(), Box<dyn std::error::Error>> {
    let config = ObservabilityConfig::default();
...
#[tokio::test]
async fn test_builtin_alert_rules() -> Result<(), Box<dyn std::error::Error>> {
...
#[tokio::test]
async fn test_observable_system_shutdown() -> Result<(), Box<dyn std::error::Error>> {
    let config = ObservabilityConfig::default();
    let system = ObservableSystem::new(config).await?;
...
#[tokio::test]
async fn test_prometheus_with_mock_metrics() -> Result<(), Box<dyn std::error::Error>> {
    let registry = Registry::new();

    // Create mock metrics
    let counter_opts = Opts::new("mock_counter", "Mock counter");
    let counter = Counter::with_opts(counter_opts)?;
    counter.inc_by(10.0);

    let gauge_opts = Opts::new("mock_gauge", "Mock gauge");
    let gauge = Gauge::with_opts(gauge_opts)?;
    gauge.set(100.5);

    let histogram_opts = HistogramOpts::new("mock_histogram", "Mock histogram");
    let histogram = HistogramVec::new(histogram_opts, &["label"])?;
...
#[tokio::test]
async fn test_observability_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let metrics_list = system.custom_metrics().await;
    assert!(metrics_list.len() > 0);
...
#[tokio::test]
async fn test_concurrent_observability_operations() -> Result<(), Box<dyn std::error::Error>> {
    let config = ObservabilityConfig::default();
    let system = ObservableSystem::new(config).await?;
...
#[tokio::test]
async fn test_observability_configurations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let system = ObservableSystem::new(minimal_config).await?;
    assert!(system.logger().is_some());
    ...
    let system = ObservableSystem::new(full_config).await?;
    assert!(system.prometheus_exporter().is_some());
    assert!(system.logger().is_some());
    assert!(system.jaeger_tracer().is_some());
    Ok(())
