...
async fn test_mobile_runtime_ios() {
    ...
    runtime.init_ios(Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(MockMobileAPI::new())))).unwrap();
...
async fn test_mobile_runtime_android() {
    let mut runtime = MobileRuntime::new();
    runtime.init_android(Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(MockJNIEnv::new())))).unwrap();
...
async fn test_wasm_runtime_creation() {
    let bee_api: _ = Arc::new(std::sync::Mutex::new(std::sync::Mutex::new(MockBeeWasmAPI)));
